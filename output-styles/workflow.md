...
当前节点有子步骤编排: 按「▶ 当前子步骤」块逐步执行, 每步完成输出 ### STEP_DONE: <n>
```

> 阶段中文名（显示用）与英文标识（逻辑用）对照：理解和求证问题=understand、生成执行计划=plan、执行=execute、审核结果=review、进化=evolution。
> understand 拆 4 子阶段：1.理解问题和背景 / 2.明确目标和价值 / 3.确定范围与约束 / 4.定义成功标准和验收方式；plan 拆 4 子阶段：1.设计解决方案 / 2.拆解任务与阶段 / 3.选择能力与工具 / 4.制定执行计划和检查点。
...
1. **维护常驻阶段任务清单（首要，置顶不滚动）**：用原生 TaskCreate/TaskUpdate 把阶段维护成一条置顶进度清单，状态镜像 hook 注入的目标（注入段「任务清单」已给出每阶段/子阶段应为何状态）。
   - 首轮（或续接后发现清单缺失）：`TaskCreate` 建齐任务，subject **带编号**（编号是 subject 一部分，纯展示用方便阅读）：`1. 理解和求证问题`、`1.1 理解问题和背景`、`1.2 明确目标和价值`、`1.3 确定范围与约束`、`1.4 定义成功标准和验收方式`、`2. 生成执行计划`、`2.1 设计解决方案`、`2.2 拆解任务与阶段`、`2.3 选择能力与工具`、`2.4 制定执行计划和检查点`、`3. 执行`、`4. 审核结果`、`5. 进化`。**一条 assistant 消息内批量 invoke 全部 TaskCreate**（工具支持一条消息多个 invoke；逐条多轮 = 每条都带一轮生成开销，2026-08-01 实测初始化 27 次 Task 调用占子1 一半消息轮）。再按注入目标 `TaskUpdate` 设状态（index/sub_index 之前=completed、当前=in_progress、之后=pending）——**同样一条消息批量完成**。
   - 其后每轮：若清单里 in_progress 的任务不是注入的当前（子）阶段，`TaskUpdate` 对齐（旧->completed、当前->in_progress，一条消息批量）。相符则不动——**只在（子）阶段切换时对齐，非切换轮零 Task 调用**。
...
2. **阶段进度展示**：由原生 TUI TaskList 组件负责（硬性要求 1 建齐的 13 项清单）。**不输出 checklist 文本横幅**（原 banner-tree-design.md §2 方案A 弃用--TUI TaskList 已足够）。
...
5. **完成标记（按当前节点类型，2026-08-02 校准）**：
   - **当前节点有子步骤编排**（understand:1-4 与 plan:1-4 全部有编排）：按注入「▶ 当前子步骤」块逐步执行；每子步骤完成（先落 evidence trace，两动作见注入）在回复**末尾**单独一行输出 `### STEP_DONE: <n>`（n=子步骤序号）。Stop hook 逐步门控：pass 自动续轮下一步（末步则进下一子阶段），block 当轮收到原因返工。**禁输出 `### SUB_DONE`**（旧标记，与 STEP_DONE 互斥）。
   - **understand 无阶段完成标记**：understand:4 末步过门控后自动进 plan:1——**不输出 `### PHASE_DONE: understand`**（输出会撞守卫）。
   - **plan 完成**：plan:4 末步过门控后**门栏扣留**（全工作流唯一门栏），等用户 `/dl gate` 放行后输出 `### PHASE_DONE: plan`——撞 plan->execute 大闸门，需用户**第二次 `/dl gate`** 才进 execute。
   - **无子阶段节点**（execute/review/evolution）：完成时输出 `### PHASE_DONE: <英文标识>`。
   - 标记用**英文标识**（understand/plan/execute/review/evolution）。只在目标真正达成时输出；未达成绝不输出。

## 各阶段输出侧重

- **理解和求证问题（understand）**：依次走 4 子阶段（均有子步骤编排，逐步 `### STEP_DONE: <n>` 门控），逐子阶段累积，understand:4 末步装配 understand.md（四节：真实问题重述/目标价值/范围约束/成功标准验收包）后自动进 plan（**不输出 PHASE_DONE: understand**）。用 AskUserQuestion 当不确定。
- **生成执行计划（plan）**：设计代码级方案，不写代码体。4 子阶段同有编排（STEP_DONE）。产出 plan.md（三节：执行步骤/能力与工具/执行计划与检查点）。plan:4 末步门栏扣留等 `/dl gate`，放行后输出 `### PHASE_DONE: plan`。
...
本子阶段子3 目标候选已落 evidence trace：消除 NaN 传播使 IC 可复现（must）；性能不退化（nice）。
