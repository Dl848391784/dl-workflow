...
1. **维护常驻阶段任务清单（首要，置顶不滚动）**：用原生 TaskCreate/TaskUpdate 把阶段维护成一条置顶进度清单，状态镜像 hook 注入的目标（注入段「任务清单」已给出每阶段/子阶段应为何状态）。
   - 首轮（或续接后发现清单缺失）：`TaskCreate` 建齐任务，**顺序**为：1.理解和求证问题、（该阶段有子阶段时紧跟）1.1理解问题和背景、1.2明确目标和价值、1.3确定范围与约束、1.4定义成功标准和验收方式、2.生成执行计划、3.执行、4.审核结果、5.进化。再按注入目标 `TaskUpdate` 设状态（index/sub_index 之前=completed、当前=in_progress、之后=pending）。
...
2. **每条回复首行**输出**全阶段进度任务清单**（checklist 格式，取代单行 ## PHASE，banner-tree-design.md §2）：
   - **方式A**：因 ark 收不到注入 attachment，清单文本由你用 `bash ~/.dl-workflow/scripts/workflow/wf-cmd.sh status` 取的状态拼（status 输出末尾含完整清单段，**可直接原样贴**）。
   - 格式（大阶段无数字前缀；当前阶段展开子阶段 2 空格缩进，其余阶段折叠）：
   ```
   ## WORKFLOW 进度 · <name>
   ▶ 理解和求证问题
     ▶ 1.1 理解问题和背景 ← 进行中
   ...
   图例: ☑=完成 ▶=进行中 ◻=待办
   ```
   - **状态语义**：▶=进行中（非完成）、☑=完成（打勾，做完才标）、◻=待办。进行中的（子）阶段绝不标 ☑。
   - **首步顺序**：首行出清单 -> 对齐原生 TaskList（首轮一次性建齐，之后不重建）-> 再做实际工作。**禁临时占位**（如"确认阶段中…"），要状态直接 `wf-cmd.sh status` 取真值。
