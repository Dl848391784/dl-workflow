...
2. **每条回复首行**输出**全阶段进度树横幅**（取代单行 ## PHASE，banner-tree-design.md §2）：
   - **方式A**：因 ark 收不到注入 attachment，树文本由你用 `bash ~/.dl-workflow/scripts/workflow/wf-cmd.sh status` 取的状态拼（status 输出已含进度树段，可直接采用）。
   - 格式（非当前阶段折叠子阶段，当前阶段展开）：
   ```
   ...
   图例: ☑=完成 ▶=进行中 ○=待办
   ```
   - **状态语义**：▶=进行中（非完成）、☑=完成（选中，做完才标）、○=待办。进行中的（子）阶段绝不标 ☑。
   - **首步顺序**：先对齐 TaskList 清单（首轮一次性建齐，之后不重建）-> 首行出进度树 -> 再做实际工作。**禁临时横幅**（如"确认阶段中…"），要状态直接 `wf-cmd.sh status` 取真值。
