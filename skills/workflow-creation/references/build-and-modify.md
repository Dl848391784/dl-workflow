...
**送达矩阵（2026-09-18「根治了吗」复核实证：修复的难点常在送达不在修复——先问「新代码怎么到达在飞实例」）**：

| 改动对象 | 在飞实例生效通道 |
|---|---|
| hooks/*.py | 每次工具调用新起子进程——pull 即生效 |
| dl_dashboard/*.py（后端） | 常驻进程内存旧码——**必须重启 server**（症状 AR） |
| dl_drive.py / build_step_prompt | driver 常驻——**重启 driver**（暂停→恢复驱动） |
| dl-launch.sh / dl-lib.sh | 下次 launch/resume 即新码 |
| per-wf settings.json（白名单等） | **版本戳 + 补写通道**：resume 落后即重写 + driver 起跑 `_maybe_refresh_settings` 自愈（v15 起两条通道都在；df393e7 前自愈链是断的——notice 只警告、resume 仅缺失补写） |
| 段 prompt / node-rules 文案 | 每段派发重新装配——新段即新文案 |
| dashboard 静态（app.js/html/css） | 版本戳 mtime 自动失效——用户硬刷一次（症状 AN） |
...
- per-wf `settings.json`（在项目 `.claude/workflows/<name>/`，非快照）改了要重启会话加载。**生命周期与 phase-rules 相反（2026-07-30 审计踩坑）**：~~resume 只在缺失时补写~~ **2026-09-18 df393e7 起双通道自愈**：resume 落后即重写（`wf_settings_stale` 判定）+ driver 起跑 `_maybe_refresh_settings`（dashboard `restart_drive` 直拉 dl_drive.py 不过 launcher，driver 侧是 dashboard 通道唯一自愈入口）。历史教训：notice 只警告指 resume 而 resume 不重写的「断路」存活了两个版本没被发现——**警告类自愈文案要验证指的路真的通**。手工刷新仍可用：`bash -c 'source ~/.dl-workflow/scripts/workflow/dl-lib.sh && wf_write_settings <name>'`（WF_META_ROOT 靠 cwd git 反查，勿在 dl-workflow 仓内跑）。对照：phase-rules.rendered.md 每次 launch/resume 重渲（改 engine purpose 即生效，见上条），无需手工。**v2.35 起有版本戳兜底**：engine `SETTINGS_TEMPLATE_VERSION` 单源，settings 盖章 `wf_settings_template_version`，workflow_phase 注入 + `/dl status` 双通道警告落后并指 `--resume`；**改 `wf_write_settings` 模板实质内容（allowlist/hooks/defaultMode）时必须 bump engine 常量**（唯一 bump 点），否则存量会话静默缴税无警告。

**新 hook/CLI 进 dl 会话的 worktree 两问（2026-09-10 cvx-wiring 实爆两条全中）**：①**模板登了吗**——dl 段会话 cwd=worktree，worktree 内无 project settings.json（`*.json` gitignore 未入库），项目级 `.claude/settings.json` 的 hook 注册在段会话**不生效**；要进工作流必须登 `wf_write_settings` 模板 + bump `SETTINGS_TEMPLATE_VERSION`（存量实例 `--resume` 补写自愈）。②**worktree 路径解析对吗**——CLI/hook 用 cwd 相对路径找项目内 db/文件（如 cvx 的 `.conventions/conventions.db`）在段会话必落空，且模型会把「找不到」当事实采信（实爆：cvx 报「无 db」→ 模型 trace 记「无活跃漂移点可比对」）；正解 = git rev-parse 反查仓根 + linked worktree 映射主仓（hooks 已有 `_map_worktree_to_main` 同构实现可抄），验证法 = `cd <任一 worktree> && 实跑该 CLI`。
...
①**教模型用的工具必须进 settings 白名单**：段 prompt/node-rules 写「invoke define-problem」而 per-wf settings allowlist 无 `Skill` 裸规则 → headless 段「Activate skill?」交互确认无人可答，3/3 全死。改编排文案提到任何工具/命令时，同 commit 查 `wf_write_settings` allowlist（+ `SETTINGS_TEMPLATE_VERSION` bump——存量 settings 靠版本戳自愈）；威胁模型=弱遵从，裸规则可接受（`AskUserQuestion`/`Agent` 同档先例）。**⚠ 但 Skill 有第二层闸，白名单救不了**（2026-09-18 GLM/qoder ×4 实爆 + 对照样本定性）：激活确认按 skill 来源分档——插件命名空间 skill（`superpowers:x`）预授信 headless 可激活，**裸名 skill（define-problem 等项目/用户级）在新 workspace 首次激活需人工确认**（每个 worktree 都是新 workspace，每实例必撞）；allow 有 `Skill` 也无效，因为拦它的是激活闸不是权限层。结构性解法 = **headless 段 skill 正文内联**（build_step_prompt 非交互分支嵌 SKILL.md 全文+禁调 Skill 工具，7e3089b；交互 TUI 段保持 invoke）——无真人通道里交互闸必须结构性不存在（症状 AL 同哲学）。判别问句：给 headless 通道加任何工具依赖前，先问「这个工具首次使用有没有交互确认闸」。②**模型面向的路径形态必须与 allowlist 同源成对**（最终形态 5fa1b11+df393e7，软链判断中间态已被取代）：`dl_flow_common.dlwf_path_forms()` 唯一函数成对输出 `{display, allow_dlcmd, allow_engine}`——display 软链等价归一（canonical/软链 → 字面 `~/.dl-workflow`；overlay/clone → 运行副本绝对路径），allow 规则两形态恒全。node-rules/段 prompt/fence 文案/engine+trace 指引/settings 白名单全部从它取；**一致性断言钉死**（测试：display 导出的规则必在生成的白名单内——结构上不可能再分叉）。两个易踩坑：`__file__.resolve()` 破解符号链接（display 判定的 canonical 比对要双侧 resolve）；bash 侧 `pwd` 默认逻辑路径（-L），含软链段与 python resolve 失配——用 `pwd -P`。根因史：白名单与展示文本曾是两个独立源头无一致性约束，dev 仓库跑 driver 时 5 连拒。

③**裁决「不做/移出」要反向 pinning**：用户裁决「X 不做/移出某档」时，用反向断言测试钉住（inject 移出 thinking 清零档：`env 不得带 MAX_THINKING_TOKENS、cmd 不得带 --thinking`，be056a4）——「不做」的决策不留痕，下个会话容易被顺手加回；反向断言把决策变成 CI 可守的边界。
