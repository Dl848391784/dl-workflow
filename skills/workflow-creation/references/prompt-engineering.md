...
- 兑现 3（怎么写→脚本）：**fetch-prompt**（外部取证子代理组装，v2.38）——curl 自由发挥 5 类事故（http+无UA 静默空/漏认证头 401/用环境性全挂的 WebFetch/手写 python 一行流语法错/原始输出污染主上下文）根除。至此**子代理 prompt 骨架模式两实例成型**（redteam/fetch），五件套：①**纪律编号**（单层/不越权/红线）；②**逐字命令模板**（本机验证版，只换查询词——弱模型不让它自由组装命令；失败按环境性/用法性/抖动性三分类对症）；③**返回契约=蒸馏边界**（行数上限+分段结构，原始输出留子代理上下文）；④**补充区**（模型只补指定区如 claim，禁手拼骨架）+ 报告**原文收录**回主会话 trace（收录项存在性机械核验——judge 会把内容丰富当实质满足放过，§3.5 #17）；⑤**摄取契约**（v2.39 补——③只约束了回传，agent 自身上下文管理是第二战场）：原始抓取一律 `| head -c <上限>` 截断或落盘再读 + 轮次上限（超限带「部分取证」返回，禁换同义词无限重试）。**动机=乘积放大**：上下文越胖，每轮请求越慢、provider 空响应重试单价越高（Q4 取证 agent 20 轮 curl 前缀 29k→60k，26 次空重试烧 1.19M in 占 90%）——设计任何子代理（含未来 plan 域 worker）时问一句：它的摄取面有界吗？
