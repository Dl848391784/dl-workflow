...
两类都在主仓库 `<项目>/.claude/evidence/<name>.jsonl`。skill-trace **v2.14 起走 `append-trace`**：载荷 = 分节标记文本（v2.58 `.trace-payload-<name>.md`，【purpose】【q】【a】零转义——模型零接触 JSON；推荐先 `append-trace --scaffold` 起骨架填「待填」，v2.57），再 Bash `dl_flow_engine.py append-trace --from-file <载荷>`——结构字段（kind/major_stage/minor_stage/sub_step/skill）脚本从 state 填、格式/路径归脚本，手写 JSONL 的 5 类事故（相对路径/覆盖/合并行/写碎/结构字段抄错）根治；直写 jsonl 被 S14 围栏 deny（v2.14 收编）。子代理报告收录走 `append-trace --ingest-agent <task-id>`（v2.60，禁手工粘贴）；产物装配走 `render-artifact`（v2.59，禁手写 understand.md/plan.md；design.md 装配已退役见 design-md-assembly-retire-design）；读回材料走 `render-readback`（v2.61）。**旧 q/a 平行数组写侧 v2.35 起硬拒**（对齐正确也拒，指路 qa 配对——过渡桥只服务凭旧惯性写的漂移模型，tail_volume plan:3 子5 q=11 a=7 实例）；evidence 记录 schema 不变（读侧不受影响）。旧「模型手写绝对路径」写法仅作历史（症状 L）。
...
### 症状 AW：dashboard 等待体感差——等待三段分解（触发拍/同步模型轮/状态翻面）

- **特征**：创建弹窗长转不关 /「交互段准备中」久等 / 提交答案到「已提交」横幅间隔数分钟——三个症状**同构**，勿各自归因瞎修。
- **分诊三段式**（按序过）：①**触发拍**——SSE 2s 一拍（代理断流走 10s 轮询兜底，症状 AV），秒级即应翻面；超过 ~15s 无变化才算触发通道卡。②**服务端同步操作**——inject/create 等 POST 内嵌**整轮模型段**（qoder 冷段 1-3min 正常；段首调必冷是已知无解项）——这是等待主体，不是 bug。③**状态翻面点**——就绪/已提交横幅依赖落盘事件（needuser 台账行收段才落 / answered.json 在 inject 模型轮跑完才落），翻面时机 = 段收工，不是用户动作完成瞬间。
- **实爆**（2026-09-17 三连）：create 弹窗长转 = launcher `--headless` 末尾 exec driver 全程跑首段绑架 POST（修=`--setup-only` 拆正建实例与驱动，456b5df）；「准备中」= prep 段+呈现段**双重会话**的呈现段全程（修=答题排队，等待与答题并行，0fec27c）；台账→横幅 2.5min = inject 模型轮本身（修=在飞态显示「注入中」，e049a43）。
- **教训**：「慢」先按三段分解再归因——把模型轮当 bug 会瞎优化，把断流当正常会瞎等。用户-facing POST 原则上不同步等模型轮（create 拆正即此原则）；**必须等时，在飞态显示是诚实底线**——「未触发」与「进行中」不可分，用户必误判为延迟。

### 症状 AI 补注（2026-09-18）

答题排队（0fec27c）把表单状态从三态扩到五态（+暂存/注入中），「状态判据要 N 态全」同规适用：暂存态须可取消/题变作废；注入中（在飞 1-3min）必须显式显示（e049a43）——缺任何一态，用户就把系统正常行为读成故障。
