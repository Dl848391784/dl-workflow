...
**链内段基线构造（2026-08-20 p2-sub2-cost）**：种子 A/B 的链内生产形态基线此前只能免跑（#30）或 fresh 化（丢链税面）——手法=把链 transcript（state.segment_chain.sid 对应 jsonl）从源实例 transcript 目录**拷入新实例 worktree 的 transcript 目录**（`~/.claude/projects/<worktree 路径编码>/`，实例名下划线连字符化），segment_chain 记录原样随 state 拷贝——`_chain_resume_sid` 三不变式全满足，段链续跑真实发生（driver 日志「⟂ 段链续跑」特征行实证，首调冷重写量级=链税直接读数）。B 轮断链生效判读=同特征行**缺席**。

**全轮无人值守顺序驱动法（2026-08-25/26 三轮因子工作流实跑：amplitude/ma5_slope/return_5d 各到 plan:4 门栏，goal 驱动）**：答案注入法（上文）从单步 A/B 扩展到**全轮无人值守**的完整 loop——①launcher `--headless` 建实例（stdin EOF 首断点自退，实例落盘即达目的，driver 残留自查）；②**预置 problem_statement 免裸开场**（`engine.set_problem_statement`——真·裸开场=u:1#1 交互步且无陈述（`_is_bare_open`），预置后该步改走 prep+needuser 段形态，无需人在场）；③`python3 dl_drive.py <name>`（无 --segment）主循环驱动，断点 stdin EOF 自退；④NEED_USER 边界读 need_user.json 注入答案（注入法同上文）；⑤到 plan:4 门栏（held_for_gate=True）即停。**四个实爆坑**：a. **注入目标 session = 当前 node/step 的 tui-step-needuser 段台账（segment_sessions 唯一权威源）**——「最新 transcript」回落两轮实爆：注进 prep 段（答非所问白烧）、注进**已完成**段（触发该步重写 2.6M cr）；b. **竞态守卫**——注入前 state 已推进（当前步查无 tui-step-needuser 记录）则**中止注入**，禁回落任何次优源（最新文件/tui_segment.json 都是坑）；c. **注入的 Stop hook 会自发拉起 driver（front 分支段派发）**——注入后先 `ps aux | grep 'dl_drive.py <name>'` 再决定手动拉，否则**双 orchestrator 实爆**（两 driver 并发写 state/抢段，kill 其一收场）；d. **后台 Bash 包装进程 ~10min 被收割（Monitor timeout，rc=143/241）≠ driver 死**——python driver 成孤儿照常推进（以 ps 为准），误判死亡再拉=人为制造双 driver。；e. **driver 探针 = 驱动，禁 timeout 强杀**（2026-09-10 cvx_wiring_e2e 实爆）——「看看 driver 在等什么」跑 `dl_drive.py <name>` 本身就是派活（execute 阶段入口判决一过直接 spawn 段）；`timeout <n>` 强杀 probe，段进程成孤儿（PPID=1 照跑、照写 worktree），下一次 drive 又 spawn 一段 = **双段并发**改同一 worktree。正确姿势：想判断点位置就让 driver 自然到断点自退（stdin EOF 即退，断点文案会打印），等不起就干脆别跑；已 orphan 的处置 = `ps` 按 PID（非 pattern）kill 孤儿段、确认零残留后干净重 drive（state/evidence 完好，#25 主条口径）；f. **wf_ctl status 的 need_user 行不判陈旧**——need_user.json 残留（交互步已过后文件不清）时 status 恒显示「有」，判活须读文件内 `node`/`sub_step` 字段对照当前 state（陈旧卡不阻塞 driver——driver 入口判决按 state 走，按它误判「在等用户」会白注入）
