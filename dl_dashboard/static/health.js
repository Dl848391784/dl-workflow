...
async function load() {
  ...
  $("gate-board").innerHTML = tbl(
    ...
    d.gate_board.map((b) =>
      ...
      `<td>${esc(b.label || b.step)}</td><td class="num">${b.judged}</td>` +
  ...
  $("node-costs").innerHTML = tbl(
    ...
    d.node_costs.map((n) =>
      `<tr><td>${esc(n.label || n.node)}</td><td class="num">${n.instances}</td>` +
  ...
  $("dispute-board").innerHTML = tbl(
    ...
    d.dispute_board.map((b) =>
      `<tr><td>${esc(b.label || b.step)}</td><td class="num">${b.count}</td></tr>`));
  if (d.errors.length) {
