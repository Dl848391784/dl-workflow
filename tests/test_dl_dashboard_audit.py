...
def _gate(node, sub_step, verdict, ts="t", reason=""):
    return json.dumps({
        "kind": "gate", "node": node, "sub_step": sub_step,
        "gate": verdict, "ts": ts, "reason": reason,
    }, ensure_ascii=False)


def _trace(minor, sub_step):
    return json.dumps({
        "kind": "skill-trace", "minor_stage": minor, "sub_step": sub_step,
        "q": ["q"], "a": ["a"],
    }, ensure_ascii=False)
...
def _mk(project: Path, name: str, evidence_lines=(), segs=(), stats_lines=()):
    ...
    (meta / "state.json").write_text(json.dumps({
        "name": name, "phase": "plan", "sub_index": 2, "node": "plan:2",
        "segment_sessions": list(segs), "force_tacet": True,
        "tacet_upgraded": ["understand:2#2"],
        "created_at": "c", "history": [],
    }), encoding="utf-8")
    if evidence_lines:
...
def test_gate_outcomes_first_pass_rate(tmp_path):
    _mk(tmp_path, "demo", evidence_lines=[
        # plan:2#3（TaskBreakdown 锚点核验，有 judge）：3 次提交 2 次 block 后过
        _trace("TaskBreakdown", 3), _gate("plan:2", 3, "blocked", "t1", "缺出处"),
        _trace("TaskBreakdown", 3), _gate("plan:2", 3, "blocked", "t2", "复合句"),
...
def test_gate_outcomes_node_gates_and_disputes(tmp_path):
    _mk(tmp_path, "demo", evidence_lines=[
        _trace("TaskBreakdown", 4),
        json.dumps({"kind": "gate", "node": "plan:2", "sub_step": 5,
                    "gate": "passed", "ts": "t8", "via": "fermate-auto-complete",
                    "gate_mech": "artifact_contains"}, ensure_ascii=False),
        json.dumps({"kind": "rubric-dispute", "node": "plan:2", "sub_step": 3,
                    "ts": "t9", "reason": "判据自相矛盾"}, ensure_ascii=False),
    ])
    g = gate_outcomes(tmp_path, "demo")
...
def test_node_costs_aggregates_by_node(tmp_path):
    segs = [
        {"ts": "t1", "session_id": "s1", "kind": "headless-step",
         "node": "plan:1", "sub_step": 1, "note": "rc=0"},
        {"ts": "t2", "session_id": "s2", "kind": "headless-step",
         "node": "plan:1", "sub_step": 2, "note": "rc=0"},
        {"ts": "t3", "session_id": "s3", "kind": "headless-step",
         "node": "plan:2", "sub_step": 1, "note": "rc=0"},
    ]
    stats = [
        json.dumps({"session_id": "s1", "num_turns": 5, "duration_ms": 60000,
                    "total_cost_usd": 0.5, "input_tokens": 1000,
                    "output_tokens": 200, "cache_read_input_tokens": 3000,
                    "cache_creation_input_tokens": 50}),
        json.dumps({"session_id": "s2", "num_turns": 3, "duration_ms": 30000,
                    "total_cost_usd": 0.25, "input_tokens": 500,
...
def test_audit_report_shape(tmp_path):
    _mk(tmp_path, "demo",
        ...
        segs=[{"ts": "t1", "session_id": "s1", "kind": "headless-step",
               "node": "plan:1", "sub_step": 1, "note": "rc=0"}])
