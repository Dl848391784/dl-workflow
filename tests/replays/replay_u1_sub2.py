...
S1 = {
    ...
    "sub_step": 1, "skill": "define-problem",
    "purpose": "逼问问题定义。",
...
VIO1["a"][1] = "年化率显示异常是因为年化计算结果过大（summary_report.json 中 ann_return=95.298 字段值，读出即事实）。"
VIO1["a"][2] = "年化计算结果过大是因为年化数值异常偏高（同一字段值 95.298 远大于 1，读出即事实）。"
VIO1["a"][3] = "年化数值异常偏高导致报告展示异常数字（报告页面显示 +9529.8%）。"
VIO1["a"][5] = "近因=「年化计算结果过大」；根因=「年化数值异常偏高」；置信度=高，每环均有数据值背书。"
VIO1["atomic_questions"] = [
    {"q": "P1 报告年化率显示 +9529.8% 异常", "tier": "none",
...
VIO2["a"][4] = "H1=用户其实想删除整个因子池、以后不再需要任何因子。排除理由：用户没有表达过这个意思，故排除 H1。"
...
VIO3["a"][1] = "量化行业一般认为 IC 均值 >0.05 才算有效因子（行业常识），0.03 低于该水平。"
VIO3["a"][2] = "该因子 IC 均值 0.03（backtest/result/default/summary_report.json 字段值，读出即事实）。"
VIO3["a"][3] = "IC 低于行业有效线导致因子预测力不足，纳入筛选会拉低组合表现。"
VIO3["a"][5] = "近因=「IC 0.03 低于行业有效线」；根因=「因子预测力不足」；置信度=中。"
VIO3["atomic_questions"] = [
    {"q": "P1 IC 均值 0.03 的因子是否达到有效水平", "tier": "none",
     "tier_reason": "答案仓内可达：IC 数值在仓内报告产物 backtest/result/default/summary_report.json，仅内查即可"}]

CASES = {"clean": CLEAN, "vio1_tautology": VIO1, "vio2_strawman": VIO2, "vio3_tier_none": VIO3}
EXPECT = {"clean": True, "vio1_tautology": False, "vio2_strawman": False, "vio3_tier_none": False}
