...
VIO1["a"][1] = (
    "【原子 B 蒸馏报告，tier=light】锚点值：业界通常认为负 IC 因子方向取反后即可"
...
VIO2["a"][1] = (
    "【原子 B 蒸馏报告，tier=light】锚点值：信息系数（IC）是衡量因子预测能力的"
...
S2_FULL_B["atomic_questions"][1]["tier_reason"] = (
    "开放设计问题（反向因子纳入筛选的方法论争议大），需五层源双向充分取证"
)
VIO3 = copy.deepcopy(
    S3_BASE
)  # 降档：标 full 按 light 参数跑（2 层源、无五层状态表），一致性核对撒谎
VIO3["purpose"] = "按档取证：A=none 仅③内查；B 派 1 个子代理轻量取证。"
VIO3["a"][3] = (
    "执行档核对：A=none 仅内查，B 按子代理取证完成（2 层源/2 curl），"
...
VIO4["a"].insert(
    2,
    "【原子 A 蒸馏报告，tier=none】锚点值：外部检索 'positive IC factor count' 命中 OpenAlex 条目（https://api.openalex.org/works?search=positive%20IC%20factor%20count），未给本仓数量；支持内查结论 14/72。",
)
VIO4["a"][4] = (
    "执行档核对：A 派了子代理补外部背景（报告已收录），B=light 按参数块执行；"
    "与子2 标称档一致；无降档、无升档。"
)

VIO5 = copy.deepcopy(S3_BASE)  # 转述冒充原文收录
VIO5["a"][1] = (
    "子代理报告大意：agent 查了 Quant StackExchange 和 OpenAlex，认为负 IC 因子"
...
VIO6["a"][1] = (
    VIO6["a"][1]
    + "【建议升档 full】SE 问答层对反向使用的争议覆盖不足，建议五层源双向补证。"
)
VIO6["a"][3] = (
    "执行档核对：A=none 仅内查，B=light 按参数块执行，与子2 标称档逐项一致。"
