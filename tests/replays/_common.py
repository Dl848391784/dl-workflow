...
def setup_env():
    ...
    assert tok.startswith("sk-"), (
        "缺 ANTHROPIC_AUTH_TOKEN——export 之，或写一行到 tests/replays/.token"
    )
...
def run_cases(title, step, label, cases, expect, n=6, gate=None):
    ...
    def one(art):
        def call(_):
            return E.run_judge(gate, label, "", artifact_content=art,
                               prior_verdicts=[], mech_scope=scope)
    ...
    for name, art in cases.items():
        ...
        res[name] = f"{good}/{n}"
        print(f"=== {name}（期望 {'PASS' if expect[name] else 'BLOCK'}，命中 {good}/{n}）===", flush=True)
