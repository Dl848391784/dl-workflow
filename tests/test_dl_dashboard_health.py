...
def _mk_instance(project: Path, name: str, gate_lines=(), segs=(), stats_lines=()):
    ...
    (meta / "state.json").write_text(json.dumps({
        ...
        "segment_sessions": list(segs), "created_at": "c", "history": [],
    }), encoding="utf-8")
    if gate_lines:
...
def _gate(node, sub_step, verdict):
    return json.dumps({"kind": "gate", "node": node, "sub_step": sub_step,
                       "gate": verdict, "ts": "t", "reason": "r"})


def _trace(minor, sub_step):
    return json.dumps({"kind": "skill-trace", "minor_stage": minor,
                       "sub_step": sub_step, "q": [], "a": []})


def test_health_aggregates_gate_board_across_instances(tmp_path):
    proj = tmp_path / "p"
    # 实例 1：plan:2#3 两连 block 后过（3 提交 2 block）；plan:3#1 一次过
    _mk_instance(proj, "wf1", [
        _trace("TaskBreakdown", 3), _gate("plan:2", 3, "blocked"),
        _trace("TaskBreakdown", 3), _gate("plan:2", 3, "blocked"),
        _trace("TaskBreakdown", 3),
        _trace("CapabilityToolSelection", 1),
    ])
    # 实例 2：plan:2#3 又一连 block 后过（2 提交 1 block）
    _mk_instance(proj, "wf2", [
        _trace("TaskBreakdown", 3), _gate("plan:2", 3, "blocked"),
        _trace("TaskBreakdown", 3),
    ])
    # 实例 3：零 gate（新实例）
...
def test_health_node_cost_percentiles(tmp_path):
    ...
    for i, cost in enumerate((0.2, 0.4, 1.0)):
        _mk_instance(
            proj, f"wf{i}",
            segs=[{"ts": "t", "session_id": f"s{i}", "kind": "headless-step",
                   "node": "plan:1", "sub_step": 1, "note": "rc=0"}],
            stats_lines=[json.dumps({
                "session_id": f"s{i}", "num_turns": 1, "duration_ms": 1000 * (i + 1),
                "total_cost_usd": cost, "input_tokens": 1, "output_tokens": 1,
