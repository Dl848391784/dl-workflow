...
<!-- END GENERATED sub_steps understand:1 -->
     ...
     - **逐步执行 + 逐步 STEP_DONE**（**写 evidence 是 STEP_DONE 前置，STEP_DONE 后 end_turn**）：
       每个子步骤达目的后，**先落 evidence 再输出 `### STEP_DONE: <n>`**。落法（两动作——你定内容，脚本管格式）：① Write 载荷 `{"purpose":"<该步目的>","qa":[{"q":...,"a":...}]}`（只含 purpose 与 qa 两个内容字段，一问一答配对成对象——不对齐在结构上不可表示；结构字段脚本从 state 自动填，不要写；**清单型产出步（归一化陈述等 record_format=statements）改用 statements 格式**：`{"purpose":...,"statements":[{"text":...,"type_label":...,"boundary":...}]}`——text 只许 outcome-level 概念，实现侧名词/file:line 只能进 boundary，append-trace 机械扫描命中即拒；具体模板以当前步注入给的示例为准）到注入给的载荷路径（`.claude/evidence/.trace-payload-<name>.json`）；② Bash `python3 ~/.dl-workflow/dl_flow_engine.py append-trace --from-file <载荷路径>`（脚本校验+单行 append 到主仓 evidence；校验失败当场报错，按报错改载荷重跑）。**禁止绕过 append-trace 手写 evidence jsonl**（手写 JSON 跨行/字面 \" = trace 隐形；直写 jsonl 会被 S14 围栏 deny 指回）。确认/裁决留痕由 /dl 命令自动写（kind=gate），不用手写其它 kind 的记录。**轮末正文总结用增量式**：首轮呈现成果可全量，**返工轮只写本轮变更条目+总数，禁全文重述**（judge 读 evidence 不读正文；完整集由读回确认步向用户呈现）。**输完 STEP_DONE 即 end_turn 结束本轮**--不连续做下个子步骤、不继续探查；**end_turn 时 Stop hook 立即门控**（读 evidence 新 trace 判定）：**非末步 pass 则当轮收到下一子步骤指令（自动续轮，直接开做下一步，无需等用户发话）**；末步 pass 时——本节点无门栏且下一子阶段有编排，**同样自动续轮进下一子阶段子1**（2026-07-27 起：无门栏的子阶段边界不是检查点，门栏才是）；门栏节点末步则扣留停轮（等用户 `/dl gate`）；block 则当轮收到原因并返工（**返工重新走①②落新行**——hook 以新 trace 为返工信号）。
