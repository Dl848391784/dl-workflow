...
- 拆 **4 子阶段**，依次完成（understand:1 有子步骤编排逐步门控，2-4 各自动推进子阶段间无闸门）：
  1. **理解问题和背景**（**子步骤编排，逐步 STEP_DONE 门控**，严格时序不可乱序）：
     ...
     - **子步骤3 = 双向取证**（designs/step3-verify-redesign-design.md；tool 步，步内多工具编排）：①主张可检验化——每个原子问题先落成可证伪 claim + 事先写死「什么证据会证实/证伪」，不可检验的退回子步骤2；②**证伪优先**——先构造反证查询（X 已解决/是反模式/不成立）并留痕，再搜支持证据；③五层源各 ≥1 次尝试留痕：学术（OpenAlex/arXiv，curl 免费 API）、社区（StackExchange/HN Algolia，curl）、开源（GitHub API，curl 带 `$GITHUB_TOKEN`）、定点网页（WebFetch 抓上述层发现的 URL）、内部仓库（codegraph+Read/Grep+Bash 查数据——既查已有解法，也证实/证伪问题在本仓真实存在）；**禁 tavily_search/WebSearch**；源层不可用显式标记「未取证+原因」是合法留痕；④codegraph 新鲜度前置——内部取证前先查索引新鲜度，>72h 先 `codegraph sync`，新鲜度结果留痕。**禁拿训练记忆冒充外部证据**（无 URL/工具留痕的「业界通常」= 编造）。
