...
## 6. 实测收官（B2：2026-08-20 10:22-10:26，L1+L2 码 bec700a，同种子复原起跑）

B2 驱动法同 B1（种子复原六件套）。代码路径核验：B2 段 prompt 内「零重验
零重跑/载荷组织钉死/每候选一对」三关键词全命中 = worktree 码生效。
**门控走通完整生产路径**：driver 正常 spawn judge → pass → 自动推进子4
（attempts=0），无手工补判（二进制修复+autoUpdates=false 生效）。

| 指标 | A（主树 f445570） | B2（L1+L2） | Δ |
|---|---|---|---|
| 首调 fresh | 34,312（cr=0） | 35,444（cr=0） | +3.3%（条款文本长度，无 strip 首调不变=预登记兑现 ✓） |
| 段 fresh 合计 | 61,669 | 48,706 | **-21.0%** |
| 段 cr 合计 | 1,268,864 | 358,272 | **-71.8%** |
| 段 out 合计 | 37,154 | 27,285 | -26.6% |
| 成本等效（fresh+0.1cr） | 188,555 | 84,533 | **-55.2%** |
| API 调用 | 19 | 8 | **-57.9%** |
| 模型墙钟 | ~329s | ~162s（首调→落库 02:23:46→02:26:28 UTC） | **-51%** |
| 工具调用 | 35 | 9 | **-74%** |
| 门控 | PASS（手工重判，driver 撞环境事故） | **PASS（driver 自动，零 block，attempts=0）** | ✓ |

B2 子3 工具序列全录（9 调用=理想最小形态）：scaffold→ls test_cases（⑤
单点）→Grep×4（②功能域对题查询：cagr/CAGR/compound、sections import、
宏 import×2）→Read 骨架→Edit→grep 待填自查→append 一次落库。**零源码
Read 重验、零 freshness 重跑、零规范文档重读、零 evidence 翻找、零 mech
拒**（L2 结构钉死消灭 B1 的 7 连拒）。

**验收逐条（§3 预登记）**：①首调 ≈34.3k±5% ✓（35,444=+3.3%，条款长度）；
②工具序列六项零全达 ✓、②查询词对题 ✓、探索类 5 ≤6 ✓；③cr -71.8% ≥40%
✓、成本等效 -55.2% ≥40% ✓；④零 block ✓（生产门控自动通过）；trace 质量
逐条自查：6 候选各①-⑤齐备+三态（可行附复用出处/假设含置信度+错误时影响）
+第 7 条材料边界自查留痕（模型主动审计自身条款合规=条款被理解执行的直接
证据）✓ 零编造 ✓；⑤pytest 1165 全绿+nodes-index 同步（L1+L2 摘要）✓；
⑥混淆声明全部按预登记处理：A 轮 Grep×3 ENOEXEC 环境噪声使 A 略偏大（方向
保守）、A 门控手工重判 vs B2 driver 自动=判侧路径等价（同 gate 文本同
artifact 裁剪）、数值 4929.2% 未触发（子3 无现状测量职责）。

**墙钟归因（#30 双轴口径）**：out -26.6% + 轮数 -58% 双降——墙钟 -51% 与
out÷rate 拟合一致（本轮无 B1 式返工褶皱，输出侧随轮数同步缩）。墙钟首达
硬线级降幅（对照 u4-sub3 B1 的输出主导持平形态：本步 L2 灭返工褶皱直接
作用于输出量）。

**沉淀**（skills 同步，见 skills/workflow-creation/references/cost-optimization.md
#35 + mechanical-checks.md）：①**条款枚举≠组织结构**——逐项枚举条款会被
弱模型镜像成载荷组织结构（按核验项拆 q），mech 逐 a 圈码齐备扫描连环拒；
组织形态必须显式钉死（purpose 预告+报错文案携带组织指引=报错即返工指令
原则的结构面应用）；②验证步复用钉死形态第三例（u:4#2/u:4#3 后），
plan:1#3 变体=台账缓存通道钉死（dl codebase query --symbol）+新鲜度判定
跨步复用（时间敏感项的例外传递：子1 实测→子3 引用）。

**遗留立项**：plan:1 子4/子5 strip/pack 逐步核对（B2 链内段数据已在
p1_sub3_ab transcript 8b036c76 留底：子4 首调 cr 暖[会话隔离缓存对同
session resume 命中？与 u:4「链恒冷」读数矛盾，待复测归因]）；交接包按步
瘦身机制（子3 不需要 understand statements 全文？无现成 per-step 机制）；
claude 自动更新器在本服务器的复发监控（autoUpdates=false 只对将来进程
生效）。amplitude 今日值 4929.2% 本轮未触发（子3 无现状测量职责）。
