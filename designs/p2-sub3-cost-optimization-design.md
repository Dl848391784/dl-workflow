...
## 5. 实测收官（2026-08-20，A=p2_sub2_ab 子3 段 / B=p2_sub3_ab 子3 段，同种子族，ac-deepseek1/deepseek-v4-flash headless）

| 指标 | A（600bfb8） | B（五杠杆） | B vs A | 预登记 | 验收 |
|---|---|---|---|---|---|
| 首调 fresh | 61,141（cr=0） | 47,415（cr=0） | **-22.4%** | ≤49,500 | ✓（strip -11.9k+条款回补=探针口径） |
| 轮数（result 权威值） | 53 | 15 | **-71.7%** | ≤22 | ✓ |
| 工具调用 | 52（Bash×48） | 14（Bash×11/Read×1/Edit×2） | **-73.1%** | ≤20 | ✓ |
| 段 fresh 合计 | 112,092 | 79,270 | -29.3% | -35% 起 | **✗ 见归因** |
| 段 cr 合计 | 1,713,536 | 420,352 | **-75.5%** | -60% 起 | ✓ |
| 成本等效（cr×0.1） | 283,445 | 121,305 | **-57.2%** | -50% 起 | ✓ 主验收轴 |
| 段 out 合计 | 51,065 | 32,719 | -35.9% | ≤40k | ✓ |
| 段 dur_api | 367.6s | 244.2s | **-33.6%** | ≤260s | ✓ |
| append-trace mech 拒 | 2 | **0**（scaffold+一次落库） | | 0 | ✓ L3 兑现 |
| 门控 | 零 block | 零 block（rc=0 一次通过） | | 零 block | ✓ |

**B 工具序列（近理想形态）**：scaffold→7 次单点例外核验（find 测试文件×1/grep 新 symbol×1/ls 目标目录×3/venv+testpaths×1/collect-only×3，全验存在即止）→Read 骨架→Edit×2→append 一次过。零全文读/零双路径/零 transcript 翻找/零宽搜掘进/零 evidence 翻找/零交付后徘徊。模型自述「8 源码文件清单、element file:line、DS callers 字段按复用钉死逐字引用」=L2 兑现。

**trace 质量逐条自查（防 Goodhart）**：U1-U5 四类核验齐备；①锚点类=逐字引用前序出处（子1 q2 文件清单/要素基线 E-ID/understand:3 IS-M1 boundary/C7.1 已证）+例外新锚点单点核验；②③=collect-only/干跑实测留痕；④声明式零命中；假设清单 H-1/H-2 附置信度+影响留子5——锚点条条有出处，执行接地零稀释 ✓。

**墙钟归因（#30 双轴）**：out÷rate——A 51,065/367.6s≈139 tok/s vs B 32,719/244.2s≈134 tok/s，同端点速率稳定，墙钟差≈输出差÷速率拟合上=输出量主导。out -35.9%=褶皱成分（mech 拒×2 返工+宽搜掘进输出）消灭而质量形态（逐字引用厚度）保留。

**fresh 合计 ✗ 归因（#33 首调 vs 合计口径教训复现）**：首调口径达标（-22.4%，机制读数），合计口径被每轮工具输出增量稀释——B 轮 collect-only×3 输出体量与 A 相同（52/45/19 tests 回显逐轮进上下文），15 轮×~2.3k 增量≈31.9k 不可压缩成分；预登记 -35% 未把「合法核验输出地板」计入=口径设定教训（同 p1-sub1 §33：合计口径目标=f(首调,轮数,工具输出地板)）。

**replay 回归（改判据必跑重放，三轮新 gate+一轮对照）**：
- 对照组（改前 gate 2644 字符）：clean 5/6 / clean2 4/6——clean2 两连 block 判词=「引用式套话缺本步命令」=预测的误 block 模式实锤；
- 新 gate（2830 字符）三轮：clean 4/6+4/6+6/6=14/18、clean2 6/6+4/6+4/6=14/18、vio1/2/3/5 三轮 18/18 全 BLOCK、vio4 0/6×3（mech 墙设计内）；
- **目标效应：引用形态误 block 模式=旧 gate 2/6 命中 → 新 gate 0/18**；clean/clean2 全部失败判词=④全局汇总发明/三态逐项发明/自相矛盾/sub_step 幻觉=既有文档化噪声模式（v2.108 落地版同带 5-6/6），无一引用新条款判 block=无系统偏移，落地。

**混淆声明**：①种子 off-by-one——state.sub_step_index 语义=「将跑之步」（1-based），种子误置 2 致 B 轮先重跑子2 一段（166s 种子误差成本，非 B 读数成分）；口径核验=A/B 两轮子2 trace 单元集（U1-U5）与文件面（12 文件）逐字相同=三查②维持；②amplitude 今日值 4929.2% 未触发——B 轮四类以外零取证条款使数据结构全文读（A 轮 amplitude JSON 结构×3=A13 掘进面）整体消灭，锚点核验不涉因子数值（与 p2-sub1 同结论）；③A/B 两臂均无 Node 白名单（main 未合入），merge 后首调再 -14.3k 叠加；④judge replay（MiniMax）与 live 门控（deepseek）分开登记。

**pytest**：1185 全绿（新增 4 例：子3 strip 生效面/复用条款 pin/gate 引用合法形态 pin/包材料不变量；既有 flags 断言更新）。

## 6. 遗留

- plan:2 子4（归一化）strip/pack 逐步核对（本分支 B 轮顺带跑过子4 段=p2-sub4-cost 免跑基线候选，三查①=子4 代码零变更）；
- 子5 读回步=零成本关闭清单核对（#22 先例）；
- fresh 合计预登记口径=首调+轮数×工具输出地板（#33 补实例）；
- main（p2-sub1 三件）合入后首调 -14.3k 白名单收益叠加确认；
- state.sub_step_index 语义（「将跑之步」1-based）写进种子组装文档——off-by-one 是种子八件套族的新坑。
