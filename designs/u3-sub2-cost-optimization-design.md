...
driver 日志无「段链续跑」行、#3/#4 各起新 session（29d6ba5f/45690f82/9f1c3b29
三段三进程）——断链代码路径确认。全 node 零 block、confirm 边界正常、
到达 u:4#1：

| 段 | 首调 fresh | 段 fresh | 段 cr | 墙钟 |
|---|---|---|---|---|
| #2（fresh 段） | 23,314 | 55,028 | 1,899,264 | 245s |
| #3（fresh 段） | **26,336** | 55,289 | **289,408** | 224s |
| #4（fresh 段） | **31,668** | 55,564 | **335,360** | 256s |

**验收对照（预登记口径 = 首调 + cr 机制读数）**：

| 指标 | 链基线（ab2/三轮复测） | 断链轮 | 变化 |
|---|---|---|---|
| #3 首调 fresh | 71,862（复测 65.3-91.8k） | 26,336 | **-63%（对复测 -60~-71%）** |
| #4 首调 fresh | 106,780（复测 92.5-122k） | 31,668 | **-70%（对复测 -66~-74%）** |
| #3+#4 段 cr | 2,340,736（ab2） | 624,768 | **-73%**（携带税灭） |
| u:3#2-4 合计 fresh | 245,384 | 165,881 | **-32%** |
| u:3#2-4 合计 cr | 3,753,472 | 2,524,032 | **-33%** |

- **混淆声明（预登记执行）**：墙钟与本体 fresh 不比（步体方差——本轮 #2
  69 轮 vs 基线 24-30 轮，#40）；$ 读数（断链 #3+#4 $2.525 vs 链 $2.327 vs
  合并 $3.474）同受本体方差混淆，不作验收。机制读数（首调 -63~-70%、
  cr -73%）足额达标。
- **行为核对**：#3 零 evidence 读取；#4 两次 evidence 访问均为定点过滤
  （sub_step 过滤 + ≤3k 字符截断，非全量翻找——宁纵勿枉的定点补合法）。
- **trace 质量**：#2-#4 全 judge pass 零 block；#2 验证留痕含规则条号
  点名与双×100 链 file:line；#4 归一化陈述携带三态标签与 in/out 边界。

### 6.6 收官结论

u:3 断链（出 CHAIN 不入 MERGED）= 本轮落地形态：#3/#4 首调 -63%/-70%
（确定性，无暖率彩票）、#3+#4 cr -73%（transcript 携带税灭）、全程零
block、trace 质量不降。合并段（续步）真实首测双边界全暖、首调 -95%，
但携带税使 cr 反超——**首调税 vs 携带税的权衡下断链 EV 最优**（§6.4）。
改动面 = dl_flow_engine.py 白名单两行 + 测试三则（链拒续/断链归属/互斥
不变量），1117 passed + ruff 绿，白名单即回滚面。
