...
B2 全节点跑通（#1-#5 全 judge pass、attempts=0 零 block）：

| 段 | A 基线首调 | B2 首调 | 变化 |
|---|---|---|---|
| u:3#2（fresh 段） | 40,228 | 25,846 | **-35.8%**（tools 白名单 -14.3k 足额） |
| u:3#3（段链冷重付） | 134,435（cr=1,792 恒冷） | 106,780（cr=1,792 恒冷） | -20.6%（白名单 + #2 探索量差混合，链税仍主导） |

附带观察（登记不定论）：B2 的 #2-#4 段链会话结束后 u:4#1 以 headless
prep 段起跑（无 NEXT_PREP stash），而基线 A（#4 独立段）有 stash——
链式段可能不携带 prep_next 顺带交付（修A lookahead 与段链的交互盲区），
独立后续项。
