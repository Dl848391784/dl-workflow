...
> 修订（2026-08-13，用户裁决）：**P1-1 验收判据修正 + P1-2 阈值校准 35k→50k**——2026-08-13 同工作流复跑审计（u:1→plan:1#1，22 步）实测首调构成：harness 地板 ~22.5k（裸 `claude -p` 实测）+ node-rules ~2-3k + 交接包本体 ~14k（分节实测：本节点前序 trace 全文 7.9k + 前序节点摘要 6.1k）+ step prompt ~2k，正常水位即 40-56k。P1-1 原目标「~30k」= 交接包预算 2-5k，**不可达（定目标时未拆地板，§3.6 #38 预算构成同族教训）**；进一步瘦身（本节点 trace 摘要化）被返工不对称否决（省 $0.01/冷启动 vs 一次 block = 2-3 轮×100k+ 重读；「按指针 Read 自取」对弱模型=症状 W 空指令；段链已把支付次数摊薄至 ~8 次/run）。判据改为：**交接包本体 ≤15k 且不随步数单调涨**（包是可控项，地板不是）。P1-2 告警对象 = 膨胀斜率，非正常水位。
...
**P2-4 相邻轻步合段（进入实施，2026-08-13 用户裁决——实施设计见 `segment-chain-resume-design.md`：会话合并非派发合并，minor_state 粒度 --resume 同会话续跑，v3.0 §2.2 两个否决理由均不触及）**。省 ~15-20 min + 0.7-0.9M fresh（deepseek 类；kimi 类 fresh 收益≈0，仅剩段启动税 4-5 min）。v3.0 §2.2 否过批量段的两个理由仍成立：append-trace state 驱动错挂步号；弱模型多步指令跟随 = 一过率对立面。只在 P1/P2 落地且护栏有数据后，挑已证明稳的步对试点（候选：u:2-u:4 的轻量连续步），不做全量铺开。
