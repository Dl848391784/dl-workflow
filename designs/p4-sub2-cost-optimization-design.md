...
> 日期：2026-08-20 · 分支 feat/p4-sub2-cost · 状态：收官（一轮 A/B 主验收轴全达）
...
## 5. 实测收官（2026-08-20 B 轮，p4_sub2_ab，ac-deepseek1/deepseek-v4-flash）

### 5.1 指标表（A=免跑基线 p2_sub3_ab plan:4#2 链内段；B=断链 fresh 段）

| 指标 | A | B | 预登记 | 判定 |
|---|---|---|---|---|
| 首调 fresh / cr | 169,338 / 0 | **50,720 / 0** | ≤56,000（-67% 起） | **✓ -70.0%** |
| 主段 cr 合计 | 1,862,144 | 716,160 | -70% 起 | ✗ -61.5%（褶皱归因见 §5.3） |
| 主段等效 | 409,297 | 182,098 | — | -55.5% |
| **合并等效（主验收轴）** | 471,095 | **182,098** | -55% 起 | **✓ -61.3%** |
| 轮数 | 12 | 10 | ≤9 | ✗（褶皱 2 轮，§5.3） |
| 工具调用（主段） | 11 | 9 | ≤7 | ✗（同上；纯税 6 全清零 ✓） |
| 红队 | fresh 61,798 / 0 工具 | **未触发=0** | 触发/未触发两态 | 未触发（附计数声明=gate 合法形态） |
| 段 out（主段） | 68,350 | 41,511 | ≤1.5× | **✓ -39.3%**（#44 形态替换判成立：禁令形态复用钉死零引用义务净增量→不放大，含断链 fresh 化仍降） |
| 段 dur_api | 537.7s | 300.8s | out÷rate 归因 | **✓ -44.1%**（41,511÷300.8≈138 tok/s vs 基线 127 tok/s，out÷rate 拟合上=输出量主导，#30） |
| 门控 | 一次通过零 block | 一次通过零 block | 零 block | **✓** |

### 5.2 机制生效实证

①**断链**：fresh spawn（新 sid 47168983，首调 cr=0）——种子 chain
清零+豁免集双保险，段形态即设计形态；②**strip**：首调 50,720 vs
同种子子1 链头 fresh spawn 58,145——包还大了 ~10k 字符（子1 trace
入包）首调反净降 7.4k=strip -11.9k 分量兑现（#23 三分量算术）；
③**pack**：包尾「本步所需材料已全部在包内」生产在场（起跑前冒烟+
B 轮 evidence 元探查零调用）；④**复用钉死**：trace q1/q3 逐字引用
TaskBreakdown sub4/CapabilityToolSelection sub5 trace（「逐字」标
注+出处），**纯税 6 调用全清零**（零 find/ls/grep locate skill 文件、
零 phase-rules 翻找、零 evidence 翻找——工具序列零探索）；⑤**交付
即止**：append 落库后零徘徊（工具序列止于落库）；⑥**红队条款**：
未触发判定附计数+理由三件套（q8），gate 合法形态一次过。

### 5.3 褶皱分解（9 调用中 3 褶皱，cr/轮数/工具三条预登记微欠的归因）

| 褶皱 | 调用 | 归因与处置 |
|---|---|---|
| Skill 误调 | #1（Skill("推理(DAG 拓扑分层 + 控制结构设计) / Agent(条件红队)")——ref 自由文本被当 skill 名） | **断链暴露行为方差新变体**（#43 族）：链内 A 轮无此动作；kind="skill"+ref 非 skill 名的组合在 fresh 段被弱模型字面化。修法候选=ref 形态或段 prompt 提示，登记 §6 |
| Write 手写载荷+自我纠正 | #4 Write 载荷（drive_mode fence 跳 S14 未拦）→ #5 重 scaffold → #6 rm+重生成 | p2-sub4-cost 同款 Write 褶皱——#40 机制解=Node 白名单去 Write：**p4-sub1-cost 已 merge（白名单五件无 Write），merge 叠加后结构性硬灭**（#40 第三验证点） |

理想最小形态 6-7 轮（Skill 误调 1+Write 循环 2-3=褶皱）与实测 10 轮
的差全在褶皱——cr -61.5% vs 预登记 -70% 起的缺口同因（褶皱轮各背
~77-84k 上下文重读），归因干净非杠杆失效。

### 5.4 trace 质量逐条自查（防 Goodhart）

9 q/a 按序对齐，形式要件全覆盖：调度四件（q1 分组附拓扑分层+同层
无依赖声明/q2 互斥面从执行包改动点字段逐字计算+组内交集=∅算式/q3
worker 任务包映射逐任务零上下文可执行/q4 返回契约三件套）/检查点
三属性（q5：3 CP，判据零判断词=gate 落库存在性断言/命令+退出码形
态，失败路由三选一预定义，类型含不可逆 op 前用户暂停）/goal
anchoring 逐 CP 两成分（q6）/密度论证=可逆性×爆炸半径逐 CP（q7，
含不采复利论证的理由）/红队未触发声明附计数+理由（q8：组 2/CP 3）
/只提案不拍板（q9）。复用钉死未稀释执行接地——提案逐项从子1 清单
推导（「逐字」出处标注在场），只是出处生产时间前移到交接包留痕。

### 5.5 混淆复盘（对照 §4 预登记八条）

①段形态差=断链杠杆本身（预登记①）✓；②红队未触发两态已分读
（主段 -55.5%/合并 -61.3% 均达轴）✓；③B 轮 launcher=本支 worktree
（机制实证②③④旁证码路径——strip 分量/尾行/条款全为 worktree 落地
件）✓；④数值漂移未触发（trace 全文未见 amplitude 数值追问）✓；
⑤褶皱 3 轮归因见 §5.3；⑥p4-sub1-cost 已先 merge 入 main（白名单/
子1 strip 不在本臂——叠加后首调预期再降 ~13.5k、Write 褶皱硬灭，
归彼臂验收）；⑦种子第九件：chain 清零+豁免集双保险，无静默失配
面；⑧看护器 pkill 误杀自身 shell（exit 144，pattern 命中命令文本）
——p4-sub1-cost #48 已收窄同坑，本轮读数无损失（裁决落盘后杀）。

### 5.6 pytest

实现落地后 1217 全绿（新增 5 例）；ruff check 零命中（format 漂移
7 文件已 filtered-patch 反应用还原，v2.119 教训）。
...
- 红队触发阈值全局未定义——本设计条款钉「附计数自主声明」与 gate
  ...
  （不改本轮 gate 零变更结论）；
- **Skill 误调变体**（§5.3）：kind="skill" + ref 自由文本（非 skill
  名）组合在 fresh 段被弱模型当 skill 名调 Skill 工具（1 轮褶皱）——
  断链暴露行为方差新变体（#43 族）；修法候选=ref 形态规整（kind=skill
  步的 ref 只写真 skill 名、推理/Agent 通道挪 purpose）或段 prompt
  提示，归框架议题（plan:3#3/plan:4#2 同构，一次性修）；
- Write 手写载荷褶皱在 drive_mode 的收口=Node 白名单去 Write
  （p4-sub1-cost 已 merge，叠加生效——#40 硬灭机制第三验证点，
  复测归后续轮顺带观察）；
