...
## 5. 实测收官（2026-08-20，A=p2_sub3_ab 子4 段 / B=p2_sub4_ab 子4 段，同种子族[种子=p2_sub3_ab evidence 截 ≤plan:2#3]，ac-deepseek1/deepseek-v4-flash headless，B 轮=worktree launcher 直调）

| 指标 | A（cbdf8ff） | B（三杠杆+白名单叠加） | B vs A | 预登记 | 验收 |
|---|---|---|---|---|---|
| 首调 fresh | 65,860（cr=0） | 34,531（cr=0） | **-47.6%** | ≤42,000（-36%） | ✓ 超额 |
| 轮数（result 权威值） | 10 | 7 | **-30.0%** | ≤9 | ✓ |
| 工具调用 | 8 | 7 | -12.5% | ≤7 | ✓ |
| 段 fresh 合计 | 113,952 | 76,174 | **-33.1%** | -20% 起 | ✓ |
| 段 cr 合计 | 631,040 | 252,928 | **-59.9%** | -30% 起 | ✓ |
| 成本等效（cr×0.1） | 177,056 | 101,466 | **-42.7%** | -25% 起 | ✓ 主验收轴 |
| 段 out 合计 | 28,909 | 27,060 | -6.4% | ±10% 内 | ✓ |
| 段 dur_api | 184.2s | 168.0s | **-8.8%** | ≤175s | ✓ |
| append-trace mech 拒 | 0 | 0 | | 0 | ✓ |
| 门控 | 零 block | 零 block（一次通过，推进 plan:3 实证） | | 零 block | ✓ |

**B 工具序列**：Skill(define-problem)→scaffold→Read 骨架→Edit→append 一次过
——前 5 调用=理论理想最小形态；Write 褶皱零发生（A 轮 S14 deny 褶皱被
main Node 白名单[无 Write]结构性消灭，软拦→硬灭实证）。落库后徘徊 2 调用
（Read plan.md[彼时未装配]+grep evidence 确认落库）=交付即止条款压制非根除
第二实例（#37 补），计褶皱 ~2 轮。

**首调归因（机制读数可分离）**：65,860→34,531=-31.3k=strip -11.9k（探针
口径）+白名单/merge 叠加 ~-14.3k+条款回补 ~+0.3k，余 ~-5.4k 有利偏差=白名单
4 工具集实测削减大于 u:2 衍生估计/merge 包渲染差——方向全对幅度超预期，
精确分解登记留后续探针。

**墙钟归因（#30 双轴）**：out÷rate——A 28,909/184.2s≈157 tok/s vs B
27,060/168.0s≈161 tok/s，拟合上=输出量主导；out -6.4%（Write deny 恢复文本
褶皱成分消灭）+轮数 -3 → 墙钟 -8.8%。out 27k=五字段+逐字引用厚度=质量形态
不撤（#30）——本步墙钟已贴近生成主导地板，再降须砍交付物厚度=质量对价，
登记留用户裁决（同 p1-sub5/p2-sub2 先例）。

**trace 质量逐条自查（防 Goodhart）**：statements 5 条五键逐键非空；要素
覆盖 E1-E15+NC-1 全承接；验收包映射 SC-G1/G3+must G2/G3+IS-M2 系列齐备；
boundary 承接逐字引用形态兑现（「子2 q2 逐字引用」「子1 q5 关系1」）；
DAG 依赖/门控条件原样携带。**一项弱化登记**：假设限定词（置信度×影响）
在 B 载荷被压缩丢失（A 轮携带「置信度高」×4，B 轮全载荷零「置信度」，
H-1/H-2/H-3 内容语义等价携带）——非本优化条款回归（④假设传导原文未动，
弱模型压缩方差），但暴露 gate 判据二灰色带：block 条件（篡改/丢失/新增）
不覆盖「内容在限定词丢」=默认-PASS framing 系统性盲区，见 §6 遗留。

**代码路径核验（#24）**：段 prompt（transcript 首条 user 消息 52,719 字符）
含「无取证例外」条款+「本步所需材料已全部在包内」包尾切换+产物清单
（understands/designs 指针）=worktree 引擎渲染实证；headless 段交接包由
driver（worktree 码）装配，包尾切换即时生效（无 #28 TUI 段 hook 侧主树
滞后问题）。

**混淆声明复盘**：①白名单 merge 叠加分量在 B 读数内——首调归因已分离
报价，主验收轴（等效 -42.7%）即使把白名单分量全扣除仍超 -25% 预登记；
②amplitude 今日值 4929.2% 未触发——零取证条款下数据文件核验面不存在
（B 工具序列零数据文件读取），预判兑现；③种子无 off-by-one（sub_step_index
=4=「将跑之步」，首段首调即子4，无重跑段）；④种子 history 修剪新坑——
history 是节点**进入日志**且 `_node_entered_at`（§8.3 产物新鲜度基准）
反查当前节点 entered_at：修剪必须保留「当前节点（plan,2）条目且
exited_at=None」，按值比较裁剪会误删（'understand'>'plan' 字典序陷阱）；
⑤B 轮过子4 后顺带跑过子5（确认级零成本，plan.md 机械装配）并进 plan:3#1
后人工停——子5/plan:3 段不进 A/B 口径。

**pytest**：1193 全绿（新增 4 例：flags 断言更新/strip+pack pin/复用条款
pin/尾行切换+包材料不变量）。

## 6. 遗留

- **plan:2 全节点成本优化收官**：子1/子2/子3/子4 完成，子5=确认级零成本
  （#22 关闭清单）——plan:2 成为首个五步全收口节点；plan:2 断链重审
  登记（u:2 MERGED 重审同款后续项）。
- 交付后徘徊第二实例（条款压制非根除）：机制杠杆候选=落库后 PreToolUse
  段尾白名单，登记。
- gate 判据二灰色带（假设限定词淡化不判）：补「淡化=限定词丢失」block
  条件+replay 双侧属 gate 改动，留用户裁决。
- 首调有利偏差 ~-5.4k 精确归因（白名单 4 工具集实测 vs u:2 衍生估计），
  留后续探针校准预登记报价。
- 种子 history 修剪规则进种子组装文档：保留当前节点进入条目+
  exited_at=None（§8.3 新鲜度基准依赖），禁按 phase 名字典序裁剪。
