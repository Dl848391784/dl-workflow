...
## 5. 实测收官（2026-08-20/21，A=p4_sub4_base 链内段 / B=p4_sub4_ab fresh 段，ac-deepseek1/deepseek-v4-flash）

### 5.1 指标表

| 指标 | A（链内段） | B（断链 fresh 段） | 预登记 | 判定 |
|---|---|---|---|---|
| 首调 fresh / cr | 248,136 / 1,024（恒冷实锤=子2+子3 transcript 冷重写） | **45,063 / 0** | ≤60,000 | **✓ -81.8%** |
| 段 fresh 合计 | 261,375 | 108,022 | — | -58.7% |
| 段 cr 合计 | 2,130,944 | 465,408 | -55% 起 | **✓ -78.2%** |
| 段等效（fresh+cr×0.1，主验收轴） | 474,469 | **154,563** | -50% 起 | **✓ -67.4%** |
| 轮数 | 10 | 10 | ≤8 | ✗ 持平（构成见 §5.3：两臂各含 1 次 mech 传导拒合法修正循环——#40 收官态步轮数不按大降预登记的实证） |
| 工具调用 | 9（Skill 误调 1+交付通道 8） | 8（Skill 正常 1+交付通道 7） | 理想最小形态 | ✓ 零探索/零翻找/零命令重跑 |
| 段 out | 26,134 | 27,079 | ≤1.9× | **✓ +3.6% 无反噬第四例**（#44 形态替换：boundary 出处指针义务原已在形式要件内，条款零引用义务净增量） |
| 段 dur_api | 171.1s | 167.4s | out÷rate 归因 | **-2.2% 墙钟中性**（out÷rate 拟合 153/162 tok/s 同带——生成主导端点+out 持平→墙钟=载荷厚度地板，#46 端点定价形态：248k 冷重写 prefill≈零墙钟，token 轴大降≠墙钟降） |
| 门控 | 1 mech 拒（缺 C7.2 传导）→修正后过，零 block | 1 mech 拒（缺 C4.3/C7.2 传导）→修正后过，零 block | 零 block | **✓**（mech 传导校验两臂各逮一次真漏=牙齿工作，合法修正循环非褶皱） |

### 5.2 机制生效实证

①**断链**：B 段新 sid 710b9c78 fresh spawn（首调 cr=0），driver 日志
无「⟂ 段链续跑」行（A 臂有=链 resume 生效对照）；②**strip**：首调
45,063 = 包 68.5k 字符（子1/2/3 trace 全文）+harness 剥后地板，在
预登记 ≤60k 内；③**pack**：包尾「本步所需材料已全部在包内」生产
在场（起跑前冒烟+B 轮零 evidence 元探查实锤）；④**复用钉死**：
B trace 7 条 statements boundary 全为「复用 子N 留痕（…逐字）」
形态（子2 提案/子3 核验出处逐字引用），工具序列零探索（零
evidence 翻找/零产物文件重读/零命令重跑）；⑤**交付即止**：append
落库后零徘徊（工具序列止于落库）。

### 5.3 行为面分解（#40 收官态确认）

A 臂 9 调用=Skill 误调 1（`Skill("define-problem(归一化)")` 带括注
全称→Unknown skill 报错，#50 变体在**链内段**也发生=归一化步固有
褶皱非断链暴露，登记框架议题）+交付通道 8（scaffold/Read/Edit×4/
append×2）；B 臂 8 调用=Skill 正常调用 1+交付通道 7。两臂各 1 次
mech 传导拒（A 缺 C7.2/B 缺 C4.3+C7.2——逐项原子化传导校验逮真漏，
修正后过）=合法修正循环。行为面除 Skill 误调外已到地板（#40），
本设计杠杆全在前缀/链层——预登记口径教训：收官态步轮数/调用按
「持平+构成改善」报价，不按大降。

### 5.4 trace 质量逐条自查（防 Goodhart）

B 轮 7 statements（调度 4+检查点 3）：text 全单句原子；fields 十键
逐键非空（调度项 cp_* 六键显式「无」/检查点项六键全填）；字段与
子2/子3 已定内容逐字可溯源（boundary「复用 子N 留痕」引用抽对：
DAG/互斥面/worker 包/返回契约=子2 留痕逐字，交集实算=子3 留痕）；
triggered 验收项落点（CP-1/CP-2 承接 SC 映射在场）；假设传导在场
（A1-A16 字段+plan.md 假设 3 条）；判据零判断词保持。复用钉死未
稀释执行接地——十字段内容量与 A 臂等价，出处生产时间前移到交接
包留痕。

### 5.5 混淆复盘（对照 §4 预登记九条）

①段形态差=断链杠杆本身 ✓；②B 种子=A 臂 evidence 裁 ≤plan:4#3
逐字同源 ✓；③数值漂移未触发（两臂 trace 无 amplitude 今日值追问，
4929.2% 备用未用）✓；④A 臂子3 段账=驱动工件未计入对比（子3 链内
段=p4_sub4_base drive-stream seg1：首调 157,527/cr=512 恒冷/34 轮/
段 cr 2.69M——p4-sub3-cost 已用同族锚点免 A 臂收官，本数据登记
备查）✓；⑤out/轮数按 fresh 形态报价 ✓（实测持平=形态替换不放大）；
⑥p4-sub3-cost 在飞件不在任一臂（两臂子3 同码 main）✓；⑦种子
第九件 chain 对齐 ✓（A 2==3-1，B 3==4-1）；⑧门栏自然收段零看护器
✓；⑨产物改名拷贝+包内产物节冒烟 ✓。
**新增事故（种子第十件）**：B 种子首跑 driver 秒退零段——
held_for_gate=True 随 A 臂末态 state 拷入（A 收于门栏扣留位形），
driver 主循环见 held 即断点等裁决。修法=state 复位 held_for_gate=
False。此前全部种子取自节点中段位形从未命中——种子装配清单补
第十件「运行时位置字段（held_for_gate/gate/pending_rework）按种子
目标位形重置」。事故轮零 token 损失（driver 秒退未 spawn 段）。

### 5.6 pytest

实现落地 1224 全绿（新增 4 例）；ruff 零命中。

## 6. 遗留立项

- **Skill 误调归一化步变体**（§5.3）：ref="define-problem(归一化)"
  带括注，弱模型按 full string 调 Skill 报 Unknown（A 臂实锤，B 臂
  模型自行用正确名=方差面）——归 #50 框架议题（plan:3#3/plan:4#2/
  plan:4#4 同构，ref 形态规整一次性修）；
- plan:4 链成员现剩子1/子5 名义在册（子2/3/4 全豁免，子5 确认级
  无会话）——节点级出册重审登记（plan:3 同型登记先例）；
- 输出侧瘦身（观察项，与各归一化步同处置——撤厚度留用户裁决）；
- 耗时轴口径结论：生成主导端点（deepseek prefill 廉价）上 token 轴
  大降≠墙钟降（本轮墙钟 -2.2% 中性）——墙钟剩余杠杆=out 厚度
  （质量成分，裁决面）或端点定价结构，登记为用户可见口径结论。
