...
| E7 | **子步骤 block 计 `node_attempts` + 连续 3 次升级为用户裁决**（2026-07-25 增补） | 原设计 block 不计数、无上限，judge 误 block 时死循环且无信号。block 时 engine 累加 `node_attempts`；达 `SUB_STEP_BLOCK_ESCALATE`(=3) 后注入改为「停止重做，AskUserQuestion 请用户裁决」：①补充信息重做 ②`/dl step-pass` 强制放行（engine `force_pass_sub_step` 写 `via=manual-step-pass` 裁决记录后按 pass 路径推进）③`/dl back` 回退。**出口是用户裁决而非放宽 rubric**（rubric 对用户是黑盒） |
...
- 子步骤编排（本节点 N 子步骤...）：
  ...
  evidence 记录（record 步必写）：向 <abs_path> 追加（每行一条 JSON）：
   {"kind":"skill-trace","major_stage":"<Phase>","minor_stage":"<MinorKey>","sub_step":<n>,"skill":"<skill>","purpose":"<该步目的>","q":["<q1>","<q2>",...],"a":["<a1>","<a2>",...]}
  字段：major_stage=phase 英文首字母大写（Understand/Plan/…）；minor_stage=子阶段英文标识（首字母大写驼峰，如 ProblemContext）；skill=子步骤调用的 skill/工具（Step.ref，如 define-problem，模型照抄注入给的当前值）；q/a=字符串数组按序对齐。
