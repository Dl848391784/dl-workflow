...
- **墙钟轴补充读数（2026-08-18 用户追问补测，两轮 transcript 时间戳）**：
  冷启动首调延迟降（#2 46s→4s 典型，prefill -27k 直接效果）；稳态逐调用
  延迟无方向性变化（12.4→13.3 / 8.5→11.0 / 17.0→10.3 / 33.8→25.4 /
  11.2→8.4~9.2，噪声带内）——稳态墙钟由生成时间+工具执行主导，非前缀
  （#13「缓存红利≠墙钟红利」同款）。**本机制定位 = token/成本轴杠杆 +
  冷启动尖峰削除；段总耗时两轮不可比（轮数方差，预登记混淆声明覆盖）**。
  u:1 墙钟残余大头 = 步体真活（挖链轮数/取证子代理运行/红队收录等待），
  已无机械空转可压。
