...
- copy `skills/workflow-creation/` -> `~/.claude/skills/`
- copy `output-styles/workflow.md` -> `~/.claude/output-styles/`
- copy `commands/wf.md` -> `~/.claude/commands/`
...
- **project settings.json 里旧 hook 注册**：本仓库把 hook 放到用户级 `~/.claude/hooks/`。若你原来在项目里也注册过（`.claude/settings.json` 里的 `python3 .claude/hooks/...`），删掉项目那份 hook 注册，用用户级注册即可。
...
- **worktree 快照铁律**：改本 repo 的 hook/output-style/scripts 后，需 `install.sh` 覆盖 `~/.claude/`；per-wf settings 用 `~/.claude/hooks/` 绝对路径，无需重建 worktree（hook 是最新版）。
