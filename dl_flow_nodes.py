...
# 仓外知识 = 问题含外部知识依赖。
# v2.77 补（2026-09-01 web_ui_interaction u:1 审计）：量级合理性问题按旧枚举
# 直落 full（635s/$2.81 外部取证，子5 三关质检 20 条证据里外部 3 条全部
# 「不承重」——主谓词由仓内数值+公式+契约闭合，量级锚点由仓内 default
# 管线对照承担）。规则内部矛盾：网关测试（不改变结论方向→none）与档位
# 枚举（量级合理性→full）打架，且 block 威胁只在漏取证侧、过度升档零
# 成本——矛盾处弱模型必选重档。修：网关测试升一票前置 + 「锚点先查」
# 正向判据 + full 档对称举证（机械校验 _FULL_TIER_JUSTIFY_RE）。
_FETCH_TIER_RULE = (
    "取证深度三档（逐原子问题定档，拿不准标 light——默认档）。"
    "定档一票前置（先于下面的档位枚举）：外部取证结论（无论证实还是"
    "证伪）是否会改变「问题是否成立」或「修复方向」？不会 → 该原子问题标 "
    "none（仓内佐证即可，不派外部 agent）——即使问题性质落在下面 full 档"
    "枚举的类别里也一样，枚举只在网关通过后才分流。"
    "锚点先查（量级/合理性类问题必经）：仓内是否已有同口径对照基线"
    "（其他管线/同族因子/历史报告的同类数值）？有 → 锚点仓内承担，外部"
    "至多 light（点查一个公认锚点数值佐证即收），不得 full。"
    ...
    "full=方法论/设计/开放问题（无单一权威、需综合；量级合理性仅当仓内"
    "确无同口径对照基线时才入此档——有对照按「锚点先查」走 light），"
    "五层源双向充分取证。tier=full 的 tier_reason 除理由外须附「仓内已查"
    "何处、确无对照基线」的举证：路径指针（文件/目录）或「已查仓内 X 无」"
    "式声明（如「已查 backtest/result/default/ 无同族对照」），与 none 档"
    "对称举证，缺此 append-trace 当场拒。"
...
_CHANGE_SPEC_RULE = (
    "改动规格条目语法（每条改动一行，类型词写括号内）："
    "<file>:<symbol>:L<a>-<b>（改|删）：<改法——改前→改后要点>；"
    "<file>:<symbol>（增@<锚点：现有 symbol|L 行号|文件尾>）：<新增内容要点>；"
    "file=git 仓内相对路径；symbol=函数名或 类.方法（模块级 import/常量填 -）；"
    "改法=改前→改后具体内容（「优化/修复」式纯动词无内容不算）；"
    "正例：src/foo.py:bar_baz:L10-12（改）：改前 X 逻辑 → 改后 Y 逻辑"
    "（行首 改=/删=/增= 前缀可写可省，写了须与括号内类型一致）；"
    "change_list/change_point 每行一条改动条目——注释/说明/假想/被否条目禁入"
    "（被否方案与理由写 rejected 字段）；增@锚点只接受三形态：现有 symbol 名 / "
    "L 行号 / 文件尾，禁自然语言描述（出处说明写 boundary 不写锚点）"
)
...
# judge 输入面（§3.5 #11）：产物文件 judge 读不到——要素原文必须
...
_TB_STEP1_FORM_REQUIREMENTS = (
    ...
    "③假设清单（含置信度×影响，原样转录）；"
    "每条附出处（evidence 指针）且要素原文引用进 trace 正文；"
...
# 本节点是首个三源聚合节点（execution-plan-checkpoints-substeps-design §1.2
# 关键不对称第八种）——聚合失真（E7）防御 = 五类清单基线 + 原文入 trace；
# judge 输入面（§3.5 #11）：plan.md/understand.md 等产物文件 judge
# 都读不到——三源原文必须引用进 trace 正文，judge 从 evidence 判。
_EPC_STEP1_FORM_REQUIREMENTS = (
    ...
    "其落点即检查点候选）；④假设清单汇总（设计包 assumptions"
    "（evidence DesignSolution trace）+ plan.md 假设项）；"
    "⑤不可逆操作候选（执行步骤中含删改/外发/force 语义的改动点）；"
    "每条附源出处且三源原文引用进 trace 正文；"
    "新增候选（三源没有的对象）显式标注或显式「无」，q/a 按序对齐；"
    "只提取不创作（本步是全节点保真判定基线）"
)
        ...
        sub_steps=(
            ...
            Step(
                ...
                purpose=(
                    ...
                    "tier 枚举/理由非空/none 档理由须含仓内路径/full 档理由须附"
                    "仓内已查无对照举证，缺项当场拒；"
                    ...
                    '\"evaluable\":[<该单元逐条可独立验收判据>]}'
                    "=源材料结构单元清单（手册各部分/栏位逐条，或用户陈述的功能块"
                    "逐条）——evaluable 必给（数组可空：模板/箴言/买卖纪律等不可"
                    "判据章节给 []）；{"
                    '\"kind\":\"req\",\"id\":\"R1\",\"req\":<需求点原文/语义>,'
                    '\"criterion\":<逐字等于所覆盖单元的某一条 evaluable 判据>,'
                    '\"covers\":[<本需求点覆盖的源结构单元>]}'
                    "=需求点。**粒度钉死：一条 req = 一个可独立验收的需求/判据**"
                    "（手册场景=逐栏逐条：八栏各一条、临界筛选各条；禁把多个可独立"
                    "成立、可分别验收的判据合并成一条——合并后「怎么实现/数据链路/"
                    "改动面」被并成一团，用户无从逐条核对完成度）；"
                    "同性质的可判据项如实拆开，不可判据章节（evaluable=[]）"
                    "可归为一条「明示不进范围」排除行（免 criterion）。"
                    "append-trace 机械校验：id 唯一、covers 逐字引用已声明单元、"
                    "**每个 source_unit 至少被一个 req 的 covers 覆盖**（漏单元=当场拒"
                    "——拆分完整性由源结构可核，非模型自觉）；"
                    "**判据↔req 1:1**（每条 evaluable 判据恰好被一条 req 的 "
                    "criterion 逐字承接：漏承接/凭空/重复承接/跨单元错位均当场拒"
                    "——粒度钉死由机械核保证，合并判据结构上写不进）。"
                ...
                selfcheck=(
                    ...
                    "拿不准标 light；none 档理由含仓内路径；full 档理由附仓内已查"
                    "无对照举证；你的论证用到的公式/"
                    "量级锚点都在仓内吗——在仓外=含外部知识依赖，不得标 none）？"
                    "atomic_questions 与 MECE 声明原子一一对应吗"
                    "（标签一致、一原子恰好一条）？"
                    "req_items 每单元 evaluable 逐条枚举了吗（不可判据章节给 []）、"
                    "每条 req 的 criterion 逐字承接一条判据、判据↔req 1:1 吗"
                    "（合并判据=结构写不进）？"
                ),
                ...
                gate=(
                    ...
                    "位置不限）；②每原子已定取证深度档。"
                    "（档枚举/理由非空/none 档仓内路径/full 档仓内已查举证/"
                    "MECE 标签对齐/req_items 结构完整与判据粒度 1:1 已由 "
                    ...
                    "标 light 且无升档空间论证判 block；full 档举证失真——"
                    "tier_reason 称「仓内无对照基线」但 q/a 论证自身已引用仓内"
                    "同口径数值作锚点（如拿仓内其他管线/同族因子数值做对比）="
                    "举证与论证自相矛盾，判 block。\n"
            ...
            Step(
                ...
                purpose=(
                    ...
                    "可消费锚点标准（plan 完备性，2026-09-10 用户 goal）：③内查凡标注"
"「供给面/先例」的条目必须带读取形态——结构（文件格式/记录示例）、"
"单位与换算、失败语义（None 还是异常）、读取先例行号；只给「存在性锚点」"
"（xxx.py:N 有一列）=欠细——plan:1/2 消费时会被迫再实读源文件（执行零求证"
"被上游击穿）。"
"模式枚举二态（禁沉默，append-trace 机械核验存在性）：根因/问题"
        ...
        sub_steps=(
            ...
            Step(
                ...
                purpose=(
                    ...
                    "codegraph 断言前置新鲜度检查，索引过期先 sync——sync 一次即止："
                    "sync 报 Already up to date 而新鲜度仍过期 = 留痕结论（codegraph"
                    "仅作符号/行号指引）继续干活，禁 force re-index/换目录重试/重复 sync"
                    "（2026-08-22 实证：同一怪癖 u:1#4 花 4 调用收场、u:3#2 烧 7 调用）；"
                    "多目标定位查询合并——无相互依赖的多个定位目标合并到单条 Bash"
                    "（grep -n -e 模式A -e 模式B 或多命令 && 串联），同文件相邻段落"
                    "一次 Read 不拆多次，逐目标单发仅限有依赖（先定位符号才能读文件）；"
                    "子1 候选自带出处（AskUserQuestion 选中项/会话事实/Read 原文）满足"
                    "gate 明文合法形态 = 逐字引用，禁为「本步留痕」重跑同类验证——"
                    "仅子1 出处不足以支撑三态处置的候选才本步补验）；"
                ...
                selfcheck=(
                    ...
                    "codegraph sync 只跑了一次吗（sync 后仍过期=留痕继续，"
                    "没 force/换目录/重试吧）？多目标定位合并成单条 Bash 了吗"
                    "（没逐目标单发吧）？子1 自带出处的候选逐字引用零重跑了吗？"
                ),
                gate=(
    ...
    "plan:1": Node(
        ...
        # 本节点无产物文件，设计包留痕在 evidence（下游 judge 读 evidence）。
        artifact=None,
        gate_mech=GateMech.NONE,
        gate_rubric=None,  # 子阶段级 rubric 删除（被 sub_steps 逐步门控取代）
        advance="sub",
        ...
        sub_steps=(
            ...
            Step(
                ...
                purpose=(
                    ...
                    "H8 2+文件需 design.md（dl-workflow 驱动改动豁免）、"
                    "H9 单次 ≤3 文件 ≤200 行可分解性、H11-H13）；⑤可测试性（TDD 前置：改动点是否存在可挂测试的接缝）。"
            ...
            Step(
                ...
                purpose=(
                    ...
                    "⑤需求覆盖核对表（coverage-table 轨道，2026-09-11 用户裁决——手册/需求侧"
"每个栏位/功能块的进/不进必须逐栏可核，防 plan 枚举静默漏项）："
"逐项之外另必给一项 {\"text\":\"需求覆盖核对表\"}，fields.coverage_table="
"全量需求条目逐行一行 `条目｜进|不进|数据缺口｜理由`——裁决三态钉死："
"进=本批实现（理由可空）；不进=本批不做（理由必给，充分性归用户 plan 门栏审）；"
"数据缺口=仓内未供给（声明外部输入+缺口形态，理由必给）。"
"不进/数据缺口行=用户裁决项——须在 plan:1#2 问答留痕中逐条有裁决记录"
"（实现 or 先不实现；静默豁免=校验缺位）。"
"④携带 verdict 边界（部分成立目标只覆盖已证实边界——裁决传导）。"
                ...
                gate=(
                    ...
                    "覆盖核对表两判（coverage-table 轨道）：①完备性——对照子1 验收包/需求陈述材料逐条核对覆盖表行，需求条目漏行（表里没有的需求条目）判 block；②用户裁决——不进/数据缺口行凡无 plan:1#2 问答留痕承接的裁决记录判 block（静默豁免=校验缺位）。合法形态=全覆盖+不进/缺口行有用户裁决即合规；理由充分性归用户 plan 门栏审，不得以「不进理由我不认可」为由 block。\n"
            ...
            Step(
                ...
                # 用户 = 无依据确认。design.md 装配已退役
                # （designs/design-md-assembly-retire-design.md：工作流内部零消费，
                # H8 按路径分流豁免）——裁决留痕在 evidence，随工作流生死。
                purpose=(
                    "带证据读回：材料 = 运行 `python3 ~/.dl-workflow/dl_flow_engine.py render-readback` 机械装配逐字呈现（本节点归一化+假设/不确定性 traces，禁手抄；Bash 输出即呈现）。呈现内容 = 推荐方案+设计包+被否方案+假设清单"
    ...
    "plan:2": Node(
        ...
        sub_steps=(
            Step(
                ...
                # 经交接包全文在场）。design.md 装配已退役（designs/
                # design-md-assembly-retire-design.md）——交接包留痕即权威出处源。
                ref="交接包前序留痕（免 evidence 翻找）",
                short="清点基线",
                purpose=(
                    ...
                    "留痕在场）。交接包留痕 = 本步唯一权威出处源（拍板内容即"
                    "DesignSolution 子5/子6 留痕原文）。"
                ),
                input="交接包（DesignSolution 子5/子6 留痕全文在包）",
                record=True,
                ...
                selfcheck=(
                    ...
                    "零 evidence 翻找/零 understand.md 读取吗？"
                ),
                gate=(
                    "evidence/<name>.jsonl 含 kind=skill-trace、"
                    ...
                    "验收包/假设是否附出处（evidence 指针或「复用 DesignSolution "
                    "子N 留痕」标注）——缺则 block；验收包/假设只需出处+简短描述即合规，"
                    "不得要求贴原文全文。\n"
                    "【判材边界】本步判材=TaskBreakdown 段 trace——交接包内容与 "
                    "DesignSolution 前步 trace judge 结构性读不到（evidence 只含 "
                    "TaskBreakdown 段）。不得以「未见设计包原文/无法核对出处真实性/"
                    "无法确认某要素是否真在设计包/无法核对验收包假设与设计包一致」"
                    "为由 block；本步只判 trace 内自洽 + 留痕形式。\n"
                    "【合法正例】「出处 复用 DesignSolution 子5 留痕，原文『在既有"
                    "聚合统计函数内增加 FACTOR_CATEGORIES 维度分组键，复用 "
                    "factor_definitions.py 映射做 group key』」合规（原文片段引用"
                    "即可）；「SC1.1『报告展示八维度条数+占比可读出』（复用 "
                    "SuccessCriteria 子4 留痕）」合规（验收包有出处+简短描述即"
                    "满足）；「H1=…（置信度中×影响中，原样转录 DesignSolution 子5 "
                    "留痕）」合规（假设原样转录+出处标注即满足）；「新增"
            ...
            Step(
                ...
                # Step strip（第十一例；第十例=子1 p2-sub1-cost）+
                # pack_self_contained（非交互步第五例）——
            ...
            Step(
                ...
                purpose=(
                    ...
                    # p2-sub3-cost（designs/p2-sub3-cost-optimization-design.md）：
                    # 复用钉死（#25 枚举例外形态）+深度钉死+④留痕形态钉死——
                    # 基线 52 调用中纯税/半税/褶皱 ~24（46%）：
                    "复用钉死：调用面/callers=交接包 DS statements callers 字段"
                    "逐字引用，零重验；要素 file:line 与文件清单=本节点子1 要素"
                    "基线留痕逐字引用，零重验；单元定义=子2 留痕全文在包，"
                    "零 evidence 重读。例外按条配额：前序零提及的锚点"
                    "（子2 新引入的文件名/测试路径/symbol）单点核验 ≤1 次/锚点，"
                    "验存在即止（ls/test -f/存在性 grep/pytest --collect-only）。"
                    "深度钉死：禁读文件全文/数据结构全文（存在性命中即止）；"
                    "禁主仓+worktree 双路径核验（主仓单路径）；"
                    "禁 transcript/会话日志翻找；锚点未命中=单点复核一次后按"
                    "三态标注（证伪回子2/假设留子5），禁宽搜掘进替前步纠错；"
                    "四类以外零取证（数据结构全文/生成物路径/覆盖率分析归 "
                    "execute）；存在性核验用显式清单单命令（禁 for 循环展开"
                    "形态）。④No Placeholders 留痕=声明式（「逐单元扫描四模式"
                    "零命中」）——禁贴扫描正则字面/模式词字面（占位词字面撞 "
                    "append-trace 占位符扫描=自伤拒收）。"
                ...
                record=True,
                fence_allow=("Bash",),
                # p2-sub3-cost：Step strip（第十二例）——四类核验全=本仓事实
                # 核验，无硬规则条号点名（p1-sub3 否决理由不适用于本步④=No
                # Placeholders 文本扫描），交付物=留痕+三态不引用自动加载
                # 文档；u4-sub3 核验步置位先例（第五例）。
                segment_strip_project_context=True,
...
默认 pass——仅当以下成立才判 block（每条附合法形态，合法形态在场不得判）：
一、声称存在无出处=编造：单元声称目标文件/symbol 存在或验证命令可运行，却无任何出处留痕判 block。违规形态：(a) 口头声称「存在/可以直接改/可以加断言/可以跑」无任何命令出处（如「`_aggregate_positive_ic` 函数就在里面，可以直接改」全段无一条命令）；(b) 四类核验整段概括复述「文件/symbol 存在、测试接缝存在、命令可运行」无任何单元特定命令。合法形态=出处留痕在场即合规——Bash 命令+返回概述（`test -f X && echo EXISTS` 返回 EXISTS / `pytest --collect-only -q` 返回 N 个用例）、codegraph 查询命令+返回概述（`codegraph callers Y` 返回 N 个调用节点）、grep 命令+返回概述、引用本节点子1/子2 trace 已载出处（原命令+返回概述逐字引用，形如「caller=1（run，子3 codegraph）」）任一形态即合规；不得索取命令输出原文/完整回显/行号/节点 ID。
二、全单元无差别「已验证」=没真核验：所有单元三态全部标「已验证」且核验留痕同形泛化（各单元文本仅复述「存在/可运行/无问题」套话，无单元特定命令/路径/返回数字）判 block。合法形态=单元级差异化留痕（每单元各自的命令+返回概述，或各自逐字引用子1/子2 trace 已载出处）即合规；多数单元已验证+个别假设项即合规；全部单元已验证但每单元附各自命令出处即合规——不得以「没有假设/证伪项」本身判 block，无证伪项不须声明「无证伪」。三态按单元标注即合规（单元整体标「已验证」覆盖该单元全部核验项），不须逐类别/逐项拆标三态，不须声明「无假设/无证伪」即完整三态--已验证单元不因「未逐项拆标/未声明无假设证伪」判 block。
...
五、漏单元核验：子2 单元集中某单元在本步无任何四类核验留痕，被「随 X 覆盖/同 X 已验证」一句话代过判 block。合法形态=每单元独立核验段（四类各自留痕，引用前序出处与新核验混合即合规）即合规；某类核验对该单元不适用（如新增常量无既有 symbol 可查）附声明即合规。
【判材边界】本步 input=step2.task_units+step1.element_baseline（S1/S2 trace 在载荷内可见）+ codegraph db/文件系统真值（结构性不可见）。只判留痕在场与 trace 内自洽，不核真值——不得以「文件是否真存在/symbol 是否真在/命令是否真能跑/返回数字是否真实」为由 block；前序留痕出处（子1/子2 trace 载荷内可见的已载命令+返回概述）的逐字引用=出处留痕在场，不得以「非本步新跑命令/出处是引用非新取证」为由 block；验证命令以 `--help`/collect-only/干跑留痕即合规，不得索取真跑单测/断言闭环/最小运行；新增常量/文件的命名冲突核验以 grep 命令+返回概述即合规，不得索取「先 grep 命名空间再声明存在」的特定顺序；三态=逐单元标注已验证/假设/证伪即合规，不得以「缺证伪分支/未声明无证伪」判 block；前步（子1/子2）的假设/断点验证方法在本步无重述义务，不得以「H1 未重述置信度/断点未复核」判 block；假设条目（三态标注中的假设态）的内容是假想陈述、非已验证事实，不须附出处/证据--不得以「假设内容断言无出处/假设描述无证据/『不破坏』类断言无核验」判 block（假设的置信度+影响字段已由 assumption_completeness_trace 机械校验）。
...
judge 判 block 须在 reason 引用判据条款并附 1 个正确改写范例（指模式不指实例位置）。"""
            ...
            Step(
                ...
                purpose=(
                    ...
                    "两条件字段（append-trace 机械校验触发即必给，缺即当场拒）："
                    "req_impl_map——纯新增批次（change_point 全为「增」）逐项必给，"
                    "一行「需求条目（承接 understand 需求陈述短句）→ 实现：<文件>.<函数>"
                    "（<机制一句>）」（技术方案「需求→实现映射」节源——纯新增功能按需求"
                    "逐条可验收，否则「完成了 100% 需求」无从核对）；"
                ...
                gate=(
                    ...
                    "合法形态=契约与子3 留痕一致即合规。\n"
                    "六、req_impl_map 与 change_point 脱节（纯新增批次）：逐项读"
                    "req_impl_map，凡映射到的「实现：文件.函数」未出现在本项"
                    "change_point 的改动清单中（写了改 A 文件却映射到 B 文件）判 block；"
                    "凡需求条目与 understand 需求陈述毫无关联（凭空条目）判 block。"
                    "合法形态=映射目标与本项 change_point 一致且需求条目承接"
                    "understand 陈述即合规——不得以「映射行文不够详细」为由 block。\n"
                    ...
                    "为由 block。设计包原文（DesignSolution trace）跨阶段结构性"
                    "读不到但子1 已提取验收包/要素为载荷内清单，不得以「无法核对"
                    "验收包与设计包一致/要素是否真在设计包」为由 block。codegraph db "
                    "不可见，不得以「无法核实 symbol 真实调用关系」为由 block"
                    "（子3 已留痕即合规）。\n"
                    ...
                    "覆盖无遗漏（置信度中×影响中，原样转录 DesignSolution 子5 留痕）」合规"
    ...
    "plan:3": Node(
        ...
        # 注入第三态读取，sub 节点不被读取）。
        advance="sub",
        # 工具白名单（plan:3 首例，节点级）——p3-sub1-cost 与 p3-sub2-cost
        # 并轨同型双侧落地（两设计独立逐步核对同结论五件）：
        # 子1/2/4=Bash+Read，子3 条件红队要 Agent（fence_allow 在册），
        # 子5 define-problem 归一化要 Skill+Edit（--scaffold 载荷填充），
        # 子6 交互步（TUI 工具自动并集/confirm 级无会话，不适用）；无
        # Grep（grep 走 Bash，plan:2 同判）、无 Write（载荷 --scaffold+Edit
        # 通道）；MCP 由 NO_MCP_ARGS 结构封死（既有）。Node 级 strip
        # 不置——子2 交付物正文引 CLAUDE.md §2 触发词（一等材料，
        # u:3#1 反优化同型，p3-sub2-cost 否决表同判），strip 只置 Step
        # 级（子1 于 p3-sub1-cost 置位，第十四例）。
        segment_tools=("Bash", "Read", "Edit", "Skill", "Agent"),
        sub_steps=(
            Step(
                kind="tool",
            ...
            Step(
                ...
                purpose=(
                    ...
                    "展开绝对路径算同一目录，禁 find/python os 复探同面）；"
                    "路由命中条目的 SKILL.md frontmatter 读取 ≤1 次/条目"
                    "（用 Read，禁 python os 批量探测）。注册表面=三通道"
                    "所列目录即止——plugins/marketplace 等其余目录不属于"
                    "本步注册表面，零探查。职责边界：CLI/MCP/环境的可用性验证"
                ...
                selfcheck=(
                    ...
                    "目录枚举同一目录累计一次了吗（~/ 与绝对路径同目录）？"
                    "plugins/marketplace 等注册表面外目录零探查了吗？"
                    "SKILL.md frontmatter ≤1 次/命中条目了吗？"
                    "有没有预跑子4 的可用性验证"
    ...
    # 去哪/谁可并行）+ 首个三源聚合节点（plan.md+understand.md+evidence——
    # 设计包在 evidence DesignSolution trace 内，design.md 装配已退役）。主敌=检查点虚设（E1 fabricated success）/误差复利（E2
    ...
    "plan:4": Node(
        ...
        sub_steps=(
            Step(
                ...
                short="三源清点",
                # 首个三源聚合节点的保真基线（同构 plan:2/3 子1）：聚合失真（E7）
                # 防御 = 五类清单 + 三源原文入 trace；triggered 验收项是检查点
                # 候选（understand:4 时机字段的消费闭合）。
                purpose=(
                    f"三源清点与追溯基线：{_EPC_STEP1_FORM_REQUIREMENTS}。"
                    "检出三源没有的对象=二次创作信号，显式列「新增候选」"
                    "待子5 用户裁决（禁静默混入）。"
                    # p4-sub1-cost L3（复用钉死③型「定点一次」三源变体，#38
                    # 判别问句：gate 出处要件钉死包外三文件行号/原文→③型；
                    # 基线实测：产物路径猎捕 ls/find/grep×13 + evidence
                    # grep/python×7=纯税，evidence 第三源 100% 已在交接包）
                    # + 交付即止（#37 平移；基线落库后预习子2 徘徊×2 实锤）
                    # + 格式真源（#26 平移）：
                    "材料边界（复用钉死）：交接包已载前序节点归一化留痕与"
                    "读回 trace 全文——evidence 源（plan:1/2/3 末步 "
                    "trace）逐字在包，直接引用即合法（「复用 <节点>子N 留痕："
                    "<出处逐字>」形态），零 evidence 全量翻找（前序 trace 已在"
                    "包内，grep evidence 通道退役）。plan.md/understand.md "
                    "两个产物文件 = 本步权威出处源：按交接包"
                ...
                input="plan.md + understand.md（指针见交接包产物清单） + "
                "交接包（plan:1/2/3 末步 trace 全文在包，设计包留痕在包）",
                record=True,
                fence_allow=("Bash",),  # scaffold/append-trace 落库通道；Read 在常驻集
                mech_checks=("epc_quote_trace",),  # v2.115：方框四原文引用下沉生产墙
                # p4-sub1-cost L1：Step 级 strip（第十八例——p3-sub5-cost 在飞
                # 占第十七，merge 复核）——交付物=五类清单+源出处+三源原文
                ...
                selfcheck=(
                    ...
                    "不可逆操作候选，无遗漏）？每条都附源出处且三源原文引用进 "
                    "trace 正文了吗（judge 读不到三个文件本身）？"
                    "triggered 验收项显式标注了吗？"
                    "新增候选显式标注或显式「无」了吗？"
                    "有静默新增三源没有的对象吗（那是二次创作）？"
                    # p4-sub1-cost L3
                    "三源材料从交接包留痕/产物定点 Read 直接引用了吗？"
                    "三文件各只 Read 一次吗？零 evidence 翻找/零路径 locate 吗？"
                ),
                gate=(
                    "evidence/<name>.jsonl 含 kind=skill-trace、minor_stage=ExecutionPlanCheckpoints 且 sub_step==1 的记录。形式要件：控制结构输入五类清单齐备（①任务 DAG 与阶段边界/②能力绑定/③验收包/④假设清单汇总/⑤不可逆操作候选）；每条附源出处且三源原文引用进 trace 正文；新增候选（三源没有的对象）显式标注或显式「无」，q/a 按序对齐；只提取不创作（本步是全节点保真判定基线）。\n"
                    "默认 pass--仅当以下成立才判 block（每条附合法形态，合法形态在场不得判）：\n"
                    "一、清单来源自证不足（编造/静默新增）：五类清单条目声称提取自三源，却既无出处（源文件+行号 / 原文『』引用）又未显式归入「新增候选」区判 block。两种违规形态：(a) 全清单裸=编造（「按三源常规结构可知…按常规路径可查」类凭印象，无任何出处/原文引用）判 block；(b) 个别条目裸且未标新增候选=静默新增二次创作（「新增候选：无」声明下清单含无出处无原文引用的新增条目，如「删除旧报告目录」类三源没有的对象）判 block。**检测：逐条检查清单中无出处（源文件+行号）且无原文『…』引用的裸条目，并核对「新增候选」答案（a[1]）是否声明「无」--「新增候选：无」声明下任何此类裸条目（无论属哪一类，⑤不可逆操作候选中的「删除旧报告目录」也不例外）= 静默新增二次创作判 block**。合法形态=源文件+行号 / 『原文』引用 任一在场即合规；「新增候选」区显式列出条目（含待子5 用户裁决语义）即合规；「新增候选：无」声明+清单无新增条目即合规--不得以「无新增候选=可能漏检」为由 block；**②能力绑定中「内置工具足够」是合法 plan.md 能力节/§2 路由结论（非新增对象，其出处随②条目整体的 plan.md:18），不得以「内置工具足够无单独出处=静默新增」为由 block**。\n"
                    "二、改写失真（聚合失真/语义偏移）：清单自述措辞与 trace 内引用的三源原文『…』明显语义冲突判 block（T1 自述「重写 _aggregate_positive_ic 为独立分组引擎」vs 其引用的原文『在既有聚合统计函数内增加 FACTOR_CATEGORIES 维度分组键』= 操作对象从「既有函数内改动」变「独立重写」、操作性质从「增加分组键」变「重写引擎」、产出物从「修改既有函数」变「全新独立引擎」，三者任一明显变化=改写失真判 block）。**检测：逐条对照每个引用了原文『…』的清单条目（含①各任务 T1/T2/T3、③各 SC），把该条自述措辞与其引用的原文三维对比--哪怕多数条目忠实提取（T2/T3/SC/H1 合规），只要个别条目自述把原文操作对象/性质/产出物明显改变（T1 自述「重写为独立分组引擎」vs 原文「在既有函数内增加分组键」）即改写失真判 block，不得以「多数条目忠实提取/整体与三源一致」为由放过个别改写条目**。合法形态=措辞是对原文的忠实提取/适度压缩/适度具体化即合规，不要求逐字一致（「增加分组键」转述为「加维度分组」类语义等价转述合法）--细节省略/语序调整/同义替换/适度具体化（原文「在既有聚合统计函数内…」自述点明函数名「`_aggregate_positive_ic` 内…」= 具体化非语义冲突）不判，操作对象/性质/产出物未变化不判。\n"
                    "三、三源漏源（五类清单缺类）：五类清单缺少某一整类（①任务 DAG/②能力绑定/③验收包/④假设汇总/⑤不可逆操作候选任一缺失）且无「该类无相关内容」声明判 block（缺③验收包整类=understand.md 在清单中无任何条目=三源之一漏源）。合法形态=五类均有条目即合规；某类无对应内容时显式标注「该类无相关内容」即合规；**某类有至少一条条目即合规，不要求该类覆盖全部源/全部子项--④仅列 H1 且声明「其余假设：无」即合规，不得以「④未列全 plan.md/evidence 假设/某类条目少/未列全全部子项/未列齐三源所有假设」为由 block**；不得以「某类条目少/粒度粗/某源仅一条」为由 block。\n"
                    "四、原文未引用：已由 epc_quote_trace 机械校验--控制结构输入清单条目引用了代码符号形（.py）却无任何『』原文引用/『原文』字样的答案已被 append-trace 当场拒、不会到你这里；你不得以「三源原文未引用/无原文/无法核对原文/原文未引用进 trace 正文」为由 block。残留判面=无代码符号形的清单条目（如纯验收包 SC ID/假设 H1 条目）是否附出处行号或『』原文引用--缺则 block；『…』摘要包裹是合法引用形态（原文片段+省略号=引用非原文整体，不要求整段逐字粘贴），不得以「『…』是节选标签式短句/非逐字完整引用/未把原文整段嵌入」为由 block。\n"
                    "【判材边界】本步 input=plan.md/understand.md（主仓 .md 文件）+ evidence plan:1/2/3 前序 trace--evidence 只含 ExecutionPlanCheckpoints 段，三源 judge 结构性读不到。不得以「未见三源原文/无法核对出处行号真实性/无法确认某条目是否真在三源/无法核对操作类型分类正确」为由 block；本步只判 trace 内自洽 + 留痕形式。三源=plan.md/understand.md/evidence（design.md 装配已退役，设计包在 evidence DesignSolution trace 内），**evidence 是一个源（plan:1/2/3 trace 的集合），清单条目引用了 evidence 任一 plan trace 即合规，不要求 plan:1/2/3 全引用/逐 trace 引用，不得以「evidence plan:2/3 trace 未引用/未声明无关联」为由 block**；五类清单=「三源实际涉及的控制结构输入对象」，不要求三源每个文件每段都引用--清单含三源承载控制结构输入的对象即合规，某源仅一条条目即合规，不要求每源条目数均衡；**不要求每子项每属性都附原文引用--每类附源出处（文件+行号）+ 至少一处『』原文片段即合规，不得以「②能力绑定未分别引用每个绑定对象/①阶段分组未引用原句/某属性无原文」为由 block**；**验收包（③）引用 understand.md 原文即合规，不要求六字段逐字全列/triggered 项逐个展开--列出 SC ID + 落点 + 原文片段即合规，不得以「六字段未全列/triggered 项未逐个标注」为由 block**；**出处行号与原文引用可集中或分散标注，不要求每子项单独附出处行号--同类多子项共享一处出处行号即合规（SC1.1/SC2.1 共享 understand.md:22、T1/T2/T3 各自或共享 plan.md 行号均合规），不得以「SC1.1 未单独附出处/出处写在类尾/出处与原文分离」为由 block**。\n"
                    "【合法正例】「①任务 DAG：T1=U2 代码改动（…增加 FACTOR_CATEGORIES 分组键，改 .py=H15 触发信号）--出处 plan.md:12，原文『U2: 在既有聚合统计函数内增加 FACTOR_CATEGORIES 维度分组键』」合规（源文件+行号+原文片段即满足，『…』片段合法）；「③验收包：SC1.1=八维度汇总区块存在性（triggered 项，落点=T3）--出处 understand.md:22，原文『SC1.1: 报告含八维度汇总区块』」合规（验收包引用 understand.md 原文即满足，不要求六字段全列）；「④假设汇总：H1=…（置信度中×影响中）--出处 evidence plan:1 DesignSolution trace，原文『假设 H1: …』」合规（evidence 任一 plan trace 引用即满足，不要求 plan:1/2/3 全引用）；「⑤不可逆操作候选：显式『无』」合规（某类无对应内容显式标注）；「新增候选：显式『无』」合规。方框以外一律不判。judge 判 block 须在 reason 引用判据条款（方框一/二/三/四）并附 1 个正确改写范例（指模式不指实例位置）。"
                ),
            ),
            Step(
                kind="skill",
                ...
                # 输入契约（step1.control_baseline 五类清单+三源原文引用）
                ...
                gate=(
                    ...
                    "【判材边界】plan.md/understand.md 产物文件 judge 结构性读不到（载荷只有本节点 trace）——出处行号/原文引用/验收包 ID 的真实性无法核对，不作 block 依据；只判 trace 内自洽+形式合规。红队条件未触发声明附计数（并行组数/检查点数）即合规，不得索阈值定义或触发论证。\n"
            ...
            Step(
                ...
                purpose=(
                    ...
                    "在三源中真实存在，codegraph/Read 出处）；"
                    "④验证手段有绑定（判据所需工具/skill 在能力包里有绑定且"
                    "无「显式不加载」冲突）。"
                    "三态标注：已验证（附出处）/假设（置信度+错误时影响）/"
                    "证伪（回子2，附理由）。只标注不裁决——"
                    "假设的接受留子5 用户裁决。"
                    # p4-sub3-cost L3：复用钉死（#39 核验步形态=出处生产时间前移
                    # +深度钉死+枚举例外按条配额）——基线大头=前序已载出处重验/
                    # 交集重复实算/三源全量翻找。核验对象 100% 源自子2 调度方案+
                    # 子1 五类清单（全文在交接包）：
                    "复用钉死：CP 判据命令/文件清单/能力绑定/验收包 SC ID="
                    "子1 五类清单+子2 调度方案留痕逐字引用，零重验（出处生产"
                    "时间前移）。例外按条配额：①互斥面交集实算=判据硬要件保留"
                    "单命令复核（python 集合交集 ≤1 次/并行组，验存在即止）；"
                    "②锚点存在性=子1 行号定点 Read/grep 单点核验 ≤1 次/锚点，"
                    "验存在即止；零 codegraph 新查询（锚点出处=子1 行号定点，"
                    "grep 兜底）。深度钉死：禁三源文件全文读（按子1 行号定点）；"
                    "禁主仓+worktree 双路径核验（主仓单路径）；禁 evidence/"
                    "transcript 翻找；锚点未命中=单点复核一次后按三态标注"
                    "（证伪回子2/假设留子5），禁宽搜掘进替前步纠错；四类以外"
                    "零取证（数据结构全文/生成物路径/覆盖率分析归 execute）。"
                    # p4-sub3-cost L5：交付即止（#37）+格式真源（#26）补款
                    # （断链暴露面同批落地，#30）：
                    "交付即止：落库成功（✓ 已落库）即结束本轮——禁 locate 产物/"
                    "读 state/grep evidence 确认落库/预习下一步，推进与门控由"
                    "外部 driver 判定。载荷格式的唯一真源 = --scaffold 骨架+"
                ...
                record=True,
                fence_allow=("Bash",),
                # p4-sub3-cost（designs/p4-sub3-cost-optimization-design.md）：
                # Step strip（第二十例——p4-sub2 占第十九）+ pack_self_contained
                # （非交互第十例）。#23 第三核对 env 剥离边界：四类核验（判据
                # dry-run/交集实算/锚点存在/验证手段绑定）全=本仓事实核验，无
                # 硬规则条号点名（p1-sub3 否决理由不适用），交付物=留痕+三态
                # 不引用自动加载文档；p2-sub3（第十二例）/p3-sub4（第十六例）
                # 核验步置位双先例。pack 置位前置=逐字段核对（设计 §2 L4）：
                # 核验对象 100% 源自子2 调度方案+子1 五类清单，全文在包。
                segment_strip_project_context=True,
                pack_self_contained=True,
                selfcheck=(
                ...
                gate=(
                    ...
                    "【判材边界】plan.md/understand.md 产物文件与 codegraph db/文件系统真值结构性不可见——只判本步 trace 内留痕在场与自洽，不核「文件/命令/锚点/工具是否真存在真能跑」；前序子1/子2 trace 在载荷内（核验对象源自 S2 调度方案：CP1/CP2/并行组/验收包 ID），子1/子2 自身方案内容/假设不重核、不索显式对照前序声明的自洽申述。\n"
            ...
            Step(
                ...
                purpose=(
                    ...
                    "trace 已在包内）、零产物文件重读（三源内容经子1 原文引用"
                ...
                gate=(
                    ...
                    '【判材边界】artifact=子1+子2+子3+子4 最新 skill-trace 拼合（生产 read_evidence_for_step(4,"ExecutionPlanCheckpoints") 同形）--sub_step==4 记录=末条，存在即满足形式要件，不得以「拼合/非独立条目/末条以子3 收尾」为由 block；前序 trace 是字段一致性对照面+验收包落点对照面（判材非纯组成事实）。plan.md/understand.md 主仓 .md 文件结构性读不到，但子1 已提取验收包 SC1/SC2 为载荷内清单--锚点存在性/出处行号/原文引用真实性不作 block 依据，只判 trace 内自洽+形式合规。子3 已留痕即合规，不得索 dry-run 通过结果或交集实算原始输出。\n'
