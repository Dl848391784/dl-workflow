        ...
        sub_steps=(
            ...
            Step(
                ...
                purpose=(
                    ...
                    "同性质的可判据项如实拆开，不可判据章节（模板/箴言/买卖纪律）"
                    "可归为一条「明示不进范围」。"
                ...
                gate=(
                    ...
                    "合法形态在场不得判）：\n"
                    "零、需求点粒度：req_items 的 req 行把多个可独立成立、可分别"
                    "验收的判据合并成一条（如「八栏①位置+②首响+③48h+⑦体量」并成"
                    "一条）判 block——合并后「怎么实现/数据链路/改动面」并成一团，"
                    "用户无从逐条核对完成度。合法形态=一条 req = 一个可独立验收的"
                    "需求/判据（手册场景逐栏逐条）；不可判据章节（模板/箴言/买卖"
                    "纪律）归为一条「明示不进范围」合规，不得以「这条没实现」为由 block。\n"
