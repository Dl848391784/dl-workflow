...
_USER_QUOTE_FORMS_RULE = (
    "「用户自述/原话」合法形态（AskUserQuestion 时代）：①自由文本原话=用户"
    "打字的话，标注「（AskUserQuestion 自由输入）」或指认直接对话轮次；"
    "②选中项标签=选项被用户选中是用户主动声明行为、合法自述，记录=标签"
    "全文+标注「（AskUserQuestion 选中）」——属会话事实级佐证，不得标"
    "「原话」（标注失真=声称的佐证等级高于实际，判 block）；"
    "③「未自述/未提及」如实标注合法（who 类）。"
    "禁止形态：纯指称无引用全文（「本会话 AskUserQuestion 回答」式）、"
    "选项标签标成「原话」、仓库事实冒充身份出处。"
    "用户只点选项时按②记录即合法——不为「原话」逼用户重复打字"
        ...
        sub_steps=(
            Step(
                ...
                gate=(
                    ...
                    f"{_USER_QUOTE_FORMS_RULE}。"
                    "（「原话」声称的通道标注已由 append-trace 机械校验——"
                    "judge 不重复判标注形态，只判内容真实性与质量。）"
                ),
            ...
            Step(
                ...
                mech_checks=("redteam_report_recorded",),
                gate=(
                    ...
                    "（已派发未收录的提前提交已由 append-trace 机械层拦截——trace "
                    "含 task-id 而无收录项不会到你这里；你若看到 task-id，收录项存在性"
                    "已机械校验，只判收录内容真实性与质量）；"
                    "verdict 与证据间推理链非跳跃；"
    ...
    "understand:2": Node(
        ...
        sub_steps=(
            Step(
                ...
                mech_checks=("value_no_unsourced_inference",),
                selfcheck=(
