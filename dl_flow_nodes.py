        ...
        sub_steps=(
            ...
            Step(
                ...
                purpose=(
                    ...
                    "（改动面取证**优先复用子2 已验证留痕**——交接包「本节点各步"
                    "最新留痕」节在场，含 callers/impact 输出与文件 file:line "
                    "清单，逐字引用并标注来源（子2/Cx 条目号）；**仅子2 未覆盖的"
                    "改动面缺口**（新 symbol/新文件）才本步新跑 codegraph impact "
                    "附留痕，CLI 路径见项目 CLAUDE.md §3 执行映射（可定向 Read）；"
                    "禁为「本步留痕」重跑前序已完成的相同查询——重复取证 = 纯税，"
                    "证据效力不增），out-of-scope = "
                ...
                selfcheck=(
                    ...
                    "改动面证据里前序（尤其子2）已验证的项都复用了吗（逐字引用"
                    "+来源标注，没为重跑而重跑）？只有缺口项才新跑 codegraph "
                    "impact 吗？"
                    "全程只提案、没替用户拍板吧？"
                ),
