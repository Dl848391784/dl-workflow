...
#
# 五步脊柱 = bug 级任务的 u/p 最小执行集（用户裁决）。阶段语义不可动：
...
# 其余 44-5=39 步在 state.force_tacet 下整步静默（driver 不派段、零 token、
# 不跑 judge、跳过交互回屏）。档位划分机制与 PIANO 本期不做（design §8）。
...
def tacet_silent_steps() -> frozenset[str]:
    """force_tacet 下整步静默的步集 = 全编排子步骤 − 五步脊柱（机械推导单源）。
