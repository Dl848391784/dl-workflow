        ...
        sub_steps=(
            Step(
                ...
                gate=(
                    ...
                    f"{_PAIN_OBSERVABILITY_RULE}。"
                    f"{_OPTION_DESIGN_RULE}。"
                    "judge 判 block 的根因是选项设计（只给认知类选项）时，"
                    "判词必须指向选项设计并给动作类选项范例，禁止只说「再追问」"
                    "——追问形式对、选项错=再追几轮都一样 block。"
                    "（「原话/自述」前缀的通道标注已由 append-trace 机械校验——"
                ...
                # 词形下沉机械层=秒拒+精确返工指路。
                mech_checks=("user_quote_channel", "answer_no_reverse_inference"),
            ),
            Step(
                kind="skill",
